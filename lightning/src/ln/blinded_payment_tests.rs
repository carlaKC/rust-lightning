...
use crate::blinded_path::payment::{
	compute_aggregated_base_prop_fee,BlindedPaymentPath, Bolt12RefundContext, DummyTlvs, ForwardTlvs, PaymentConstraints,
...
use crate::events::{Event, HTLCHandlingFailureType, HTLCLocator, PaymentFailureReason};
...
fn create_blinded_tail(
	...
	fred_cltv_final: u32, final_value_msat: u64, payment_secret: PaymentSecret,
) -> BlindedTail {
	let outer_session_priv = SecretKey::from_slice(&override_random_bytes).unwrap();
	let trampoline_session_priv = onion_utils::compute_trampoline_session_priv(&outer_session_priv);
...
fn replacement_onion(
	...
	route: Route, fred_amt_msat: u64, fred_final_cltv: u32, payment_hash: PaymentHash,
	payment_secret: PaymentSecret, blinded: bool, starting_htlc_offset: u32, carol: PublicKey,
	eve: (PublicKey, &PaymentRelay), fred: PublicKey,
) -> msgs::OnionPacket {
			...
			trampoline_payloads = vec![
				...
				msgs::OutboundTrampolinePayload::Forward {
					...
					outgoing_cltv_value: starting_htlc_offset
						+ fred_final_cltv + eve.1.cltv_expiry_delta as u32,
				...
				msgs::OutboundTrampolinePayload::Forward {
					...
					outgoing_cltv_value: starting_htlc_offset + fred_final_cltv,
				...
				msgs::OutboundTrampolinePayload::Receive {
					...
					cltv_expiry_height: starting_htlc_offset + fred_final_cltv,
				},
			];
		}
...
fn do_test_trampoline_relay(blinded: bool, test_case: TrampolineTestCase) {
	...
	let eve_trampoline_relay = PaymentRelay {
		// Note: We add 1 here so that there's a non-zero forwarding cltv budget for
		// eve to forward to Dave - our pathfinding doesn't currently allow a zero value
		// and would fail with no path.
	...
	*nodes[0].keys_manager.override_random_bytes.lock().unwrap() = Some(override_random_bytes);
	...
	let cltv_bytes = (fred_cltv_final + starting_htlc_offset).to_be_bytes();
	...
	if let Some(failure) = payment_failure {
		...
		expect_and_process_pending_htlcs_and_htlc_handling_failed(
			...
			&[HTLCHandlingFailureType::TrampolineForward {
				attempted_htlcs: vec![HTLCLocator {
					channel_id: eve_fred_chan.2,
					user_channel_id: None,
					node_id: Some(fred_node_id),
				}],
			}],
		);
		...
		expect_and_process_pending_htlcs_and_htlc_handling_failed(
			...
			&[HTLCHandlingFailureType::TrampolineForward {
				attempted_htlcs: vec![HTLCLocator {
					channel_id: carol_dave_chan.2,
					user_channel_id: None,
					node_id: Some(dave_node_id),
				}],
			}],
		);
