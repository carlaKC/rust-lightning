	...
	<SP::Target as SignerProvider>::EcdsaSigner: EcdsaChannelSigner
{
	...
	{
		if self.context.channel_state.is_local_shutdown_sent() {
			return Err(("Shutdown was already sent", LocalHTLCFailure::ShutdownSent))
		}
		...
		if on_counterparty_tx_dust_htlc_exposure_msat > max_dust_htlc_exposure_msat {
			...
			return Err(("Exceeded our total dust exposure limit on counterparty commitment tx", LocalHTLCFailure::DustLimitCounterparty))
		}
		...
		if msg.amount_msat / 1000 < exposure_dust_limit_success_sats {
			...
			if on_holder_tx_dust_htlc_exposure_msat > max_dust_htlc_exposure_msat {
				...
				return Err(("Exceeded our dust exposure limit on holder commitment tx", LocalHTLCFailure::DustLimitHolder))
			}
		}
		...
		if !self.funding.is_outbound() {
			...
			if pending_remote_value_msat.saturating_sub(self.funding.holder_selected_channel_reserve_satoshis * 1000).saturating_sub(anchor_outputs_value_msat) < remote_fee_cost_incl_stuck_buffer_msat {
				...
				return Err(("Fee spike buffer violation", LocalHTLCFailure::FeeSpikeBuffer));
			}
		}
