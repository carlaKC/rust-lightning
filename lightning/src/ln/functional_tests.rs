...
use crate::events::{Event, FundingInfo, PathFailure, PaymentPurpose, ClosureReason, HTLCHandlingType, PaymentFailureReason};
...
pub fn test_async_inbound_update_fee() {
	...
	nodes[0].node.handle_commitment_signed_batch_test(nodes[1].node.get_our_node_id(), &payment_event.commitment_msg); // (2)
	...
	nodes[0].node.handle_commitment_signed_batch_test(nodes[1].node.get_our_node_id(), &bs_update.commitment_signed); // deliver (4)
	...
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &as_update.commitment_signed); // deliver (5)
...
pub fn test_multi_flight_update_fee() {
	...
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), commitment_signed_1);
	...
	// Delever (4)
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &as_second_update.commitment_signed);
	...
	nodes[0].node.handle_commitment_signed_batch_test(nodes[1].node.get_our_node_id(), &bs_second_commitment);
	let as_second_revoke = get_event_msg!(nodes[0], MessageSendEvent::SendRevokeAndACK, nodes[1].node.get_our_node_id());
	// No commitment_signed so get_event_msg's assert(len == 1) passes
	check_added_monitors!(nodes[0], 1);
...
pub fn test_update_fee_with_fundee_update_add_htlc() {
	...
	nodes[0].node.handle_commitment_signed_batch_test(nodes[1].node.get_our_node_id(), &commitment_update.commitment_signed);
	check_added_monitors!(nodes[0], 1);
...
pub fn test_update_fee() {
	...
	// Handle (3), creating (6):
	nodes[0].node.handle_commitment_signed_batch_test(nodes[1].node.get_our_node_id(), &commitment_signed_0);
	check_added_monitors!(nodes[0], 1);
...
pub fn test_duplicate_htlc_different_direction_onchain() {
	...
	for e in events {
		match e {
			...
			MessageSendEvent::UpdateHTLCs { ref node_id, channel_id: _, updates: msgs::CommitmentUpdate { ref update_add_htlcs, ref update_fulfill_htlcs, ref update_fail_htlcs, ref update_fail_malformed_htlcs, .. } } => {
...
pub fn test_fee_spike_violation_fails_htlc() {
	...
	expect_pending_htlcs_forwardable!(nodes[1]);
	expect_htlc_handling_failed!(nodes[1].node.get_and_clear_pending_events(), &[HTLCHandlingType::ReceiveFailed { payment_hash }]);
...
pub fn test_channel_reserve_holding_cell_htlcs() {
	...
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &payment_event_1.commitment_msg);
...
pub fn channel_reserve_in_flight_removes() {
	...
	nodes[0].node.handle_commitment_signed_batch_test(nodes[1].node.get_our_node_id(), &bs_removes.commitment_signed);
	check_added_monitors!(nodes[0], 1);
	let as_raa = get_event_msg!(nodes[0], MessageSendEvent::SendRevokeAndACK, nodes[1].node.get_our_node_id());
	expect_payment_sent(&nodes[0], payment_preimage_1, None, false, false);

	nodes[1].node.handle_update_add_htlc(nodes[0].node.get_our_node_id(), &send_1.msgs[0]);
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &send_1.commitment_msg);
	check_added_monitors!(nodes[1], 1);
	...
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &as_cs.commitment_signed);
	check_added_monitors!(nodes[1], 1);
	let bs_raa = get_event_msg!(nodes[1], MessageSendEvent::SendRevokeAndACK, nodes[0].node.get_our_node_id());
	...
	nodes[0].node.handle_commitment_signed_batch_test(nodes[1].node.get_our_node_id(), &bs_cs.commitment_signed);
	check_added_monitors!(nodes[0], 1);
	let as_raa = get_event_msg!(nodes[0], MessageSendEvent::SendRevokeAndACK, nodes[1].node.get_our_node_id());
	...
	nodes[0].node.handle_commitment_signed_batch_test(nodes[1].node.get_our_node_id(), &send_2.commitment_msg);
	check_added_monitors!(nodes[0], 1);
	let as_raa = get_event_msg!(nodes[0], MessageSendEvent::SendRevokeAndACK, nodes[1].node.get_our_node_id());
		...
		($node: expr, $prev_node: expr, $preimage: expr, $payment_hash: expr) => {
			{
				...
				assert_eq!(events.len(), 1);
				match events[0] {
					MessageSendEvent::UpdateHTLCs { ref node_id, channel_id: _, updates: msgs::CommitmentUpdate { ref update_add_htlcs, ref update_fail_htlcs, .. } } => {
						assert!(update_add_htlcs.is_empty());
						assert!(update_fail_htlcs.is_empty());
...
pub fn test_multiple_package_conflicts() {
	...
	nodes[0]
		.node
		.handle_commitment_signed_batch_test(nodes[1].node.get_our_node_id(), &updates.commitment_signed);
	check_added_monitors(&nodes[0], 1);
	...
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &commit_signed);
...
pub fn test_simple_commitment_revoked_fail_backward() {
	...
	assert_eq!(events.len(), 1);
	match events[0] {
		MessageSendEvent::UpdateHTLCs { ref node_id, channel_id: _, updates: msgs::CommitmentUpdate { ref update_add_htlcs, ref update_fail_htlcs, ref update_fulfill_htlcs, ref update_fail_malformed_htlcs, ref commitment_signed, .. } } => {
			assert!(update_add_htlcs.is_empty());
			assert_eq!(update_fail_htlcs.len(), 1);
			assert!(update_fulfill_htlcs.is_empty());
			assert!(update_fail_malformed_htlcs.is_empty());
...
fn do_test_commitment_revoked_fail_backward_exhaustive(deliver_bs_raa: bool, use_dust: bool, no_to_remote: bool) {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[2], vec![HTLCHandlingType::ReceiveFailed { payment_hash: first_payment_hash }]);
	check_added_monitors!(nodes[2], 1);
	let updates = get_htlc_update_msgs!(nodes[2], nodes[1].node.get_our_node_id());
	assert!(updates.update_add_htlcs.is_empty());
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[2], vec![HTLCHandlingType::ReceiveFailed { payment_hash: second_payment_hash }]);
	check_added_monitors!(nodes[2], 1);
	let updates = get_htlc_update_msgs!(nodes[2], nodes[1].node.get_our_node_id());
	assert!(updates.update_add_htlcs.is_empty());
	assert!(updates.update_fulfill_htlcs.is_empty());
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[2], vec![HTLCHandlingType::ReceiveFailed { payment_hash: third_payment_hash }]);
	check_added_monitors!(nodes[2], 1);
	let updates = get_htlc_update_msgs!(nodes[2], nodes[1].node.get_our_node_id());
	assert!(updates.update_add_htlcs.is_empty());
	assert!(updates.update_fulfill_htlcs.is_empty());
...
pub fn test_force_close_fail_back() {
	...
	nodes[2].node.force_close_broadcasting_latest_txn(&channel_id, &nodes[1].node.get_our_node_id(), error_message.to_string()).unwrap();
	check_closed_broadcast!(nodes[2], true);
	check_added_monitors!(nodes[2], 1);
	...
	{
		get_monitor!(nodes[2], channel_id)
	...
	if messages_delivered >= 1 {
		...
		if messages_delivered >= 2 {
			...
			if messages_delivered >= 3 {
				...
				if messages_delivered >= 4 {
					nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &as_commitment_signed);
					let bs_revoke_and_ack = get_event_msg!(nodes[1], MessageSendEvent::SendRevokeAndACK, nodes[0].node.get_our_node_id());
					// No commitment_signed so get_event_msg's assert(len == 1) passes
					check_added_monitors!(nodes[1], 1);
...
pub fn test_drop_messages_peer_disconnect_dual_htlc() {
	...
	assert_eq!(events_2.len(), 1);
	match events_2[0] {
		MessageSendEvent::UpdateHTLCs { ref node_id, channel_id: _, updates: msgs::CommitmentUpdate { ref update_add_htlcs, ref update_fulfill_htlcs, ref update_fail_htlcs, ref update_fail_malformed_htlcs, ref update_fee, ref commitment_signed } } => {
			assert_eq!(*node_id, nodes[0].node.get_our_node_id());
			assert!(update_add_htlcs.is_empty());
			assert_eq!(update_fulfill_htlcs.len(), 1);
			assert!(update_fail_htlcs.is_empty());
...
fn do_test_fail_backwards_unrevoked_remote_announce(deliver_last_raa: bool, announce_latest: bool) {
	...
	let failed_destinations = vec![
		HTLCHandlingType::ReceiveFailed { payment_hash: payment_hash_1 },
		HTLCHandlingType::ReceiveFailed { payment_hash: payment_hash_3 },
		HTLCHandlingType::ReceiveFailed { payment_hash: payment_hash_5 },
		HTLCHandlingType::ReceiveFailed { payment_hash: payment_hash_6 },
	];
	...
	let failed_destinations_2 = vec![
		HTLCHandlingType::ReceiveFailed { payment_hash: payment_hash_2 },
		HTLCHandlingType::ReceiveFailed { payment_hash: payment_hash_4 },
	];
	...
	let failed_destinations_3 = vec![
		HTLCHandlingType::ForwardFailed { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2 },
		HTLCHandlingType::ForwardFailed { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2 },
		HTLCHandlingType::ForwardFailed { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2 },
		HTLCHandlingType::ForwardFailed { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2 },
		HTLCHandlingType::ForwardFailed { node_id: Some(nodes[5].node.get_our_node_id()), channel_id: chan_3_5.2 },
		HTLCHandlingType::ForwardFailed { node_id: Some(nodes[5].node.get_our_node_id()), channel_id: chan_3_5.2 },
	];
	...
	if deliver_last_raa {
		...
		let expected_destinations: Vec<HTLCHandlingType> = repeat(HTLCHandlingType::ForwardFailed { node_id: Some(nodes[3].node.get_our_node_id()), channel_id: chan_2_3.2 }).take(3).collect();
		expect_htlc_handling_failed!(nodes[2].node.get_and_clear_pending_events(), expected_destinations);
	} else {
		let expected_destinations: Vec<HTLCHandlingType> = if announce_latest {
			repeat(HTLCHandlingType::ForwardFailed { node_id: Some(nodes[3].node.get_our_node_id()), channel_id: chan_2_3.2 }).take(9).collect()
		} else {
			repeat(HTLCHandlingType::ForwardFailed { node_id: Some(nodes[3].node.get_our_node_id()), channel_id: chan_2_3.2 }).take(6).collect()
		};
...
fn do_htlc_claim_previous_remote_commitment_only(use_dust: bool, check_revoke_no_close: bool) {
	...
	nodes[1].node.handle_revoke_and_ack(nodes[0].node.get_our_node_id(), &as_updates.0);
	check_added_monitors!(nodes[1], 1);
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &as_updates.1);
	check_added_monitors!(nodes[1], 1);
	let bs_revoke_and_ack = get_event_msg!(nodes[1], MessageSendEvent::SendRevokeAndACK, nodes[0].node.get_our_node_id());
...
pub fn test_update_add_htlc_bolt2_sender_exceed_max_htlc_num_and_htlc_id_increment() {
	...
	for i in 0..max_accepted_htlcs {
		...
		let payment_event = {
			...
			let mut events = nodes[0].node.get_and_clear_pending_msg_events();
			assert_eq!(events.len(), 1);
			if let MessageSendEvent::UpdateHTLCs { node_id: _, channel_id: _, updates: msgs::CommitmentUpdate{ update_add_htlcs: ref htlcs, .. }, } = events[0] {
...
pub fn test_update_fulfill_htlc_bolt2_incorrect_htlc_id() {
	...
	let mut update_fulfill_msg: msgs::UpdateFulfillHTLC = {
		match events[0] {
			MessageSendEvent::UpdateHTLCs { node_id: _, channel_id: _, updates: msgs::CommitmentUpdate { ref update_add_htlcs, ref update_fulfill_htlcs, ref update_fail_htlcs, ref update_fail_malformed_htlcs, ref update_fee, .. } } => {
				assert!(update_add_htlcs.is_empty());
				assert_eq!(update_fulfill_htlcs.len(), 1);
				assert!(update_fail_htlcs.is_empty());
				assert!(update_fail_malformed_htlcs.is_empty());
...
pub fn test_update_fulfill_htlc_bolt2_missing_badonion_bit_for_malformed_htlc_message() {
	...
	expect_pending_htlcs_forwardable!(nodes[1]);
	expect_htlc_handling_failed!(nodes[1].node.get_and_clear_pending_events(), &[HTLCHandlingType::InvalidOnion]);
	check_added_monitors(&nodes[1], 1);
...
pub fn test_update_fulfill_htlc_bolt2_after_malformed_htlc_message_must_forward_update_fail_htlc() {
	...
	expect_pending_htlcs_forwardable!(nodes[2]);
	expect_htlc_handling_failed!(nodes[2].node.get_and_clear_pending_events(), &[HTLCHandlingType::InvalidOnion]);
	check_added_monitors(&nodes[2], 1);
...
fn do_test_failure_delay_dust_htlc_local_commitment(announce_latest: bool) {
	...
	check_added_monitors!(nodes[1], 0);
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1], vec![HTLCHandlingType::ReceiveFailed { payment_hash: payment_hash_2 }]);
	check_added_monitors!(nodes[1], 1);
	...
	nodes[0].node.handle_commitment_signed_batch_test(nodes[1].node.get_our_node_id(), &remove.commitment_signed);
	check_added_monitors!(nodes[0], 1);
...
pub fn test_bump_penalty_txn_on_revoked_htlcs() {
	...
	let events = nodes[0].node.get_and_clear_pending_events();
	expect_pending_htlcs_forwardable_conditions(events[0..2].to_vec(), &[HTLCHandlingType::ReceiveFailed { payment_hash: failed_payment_hash }]);
...
pub fn test_bump_txn_sanitize_tracking_maps() {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed_ignore!(nodes[0], vec![HTLCHandlingType::ReceiveFailed { payment_hash: payment_hash_2 }]);
	assert_eq!(nodes[0].tx_broadcaster.txn_broadcasted.lock().unwrap().len(), 0);
...
pub fn test_bad_secret_hash() {
	...
	macro_rules! handle_unknown_invalid_payment_data {
		($payment_hash: expr) => {
			...
			expect_pending_htlcs_forwardable!(nodes[1]);
			expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1], vec![HTLCHandlingType::ReceiveFailed{ payment_hash: $payment_hash }]);
			check_added_monitors!(nodes[1], 1);

			// We should fail the payment back
			let mut events = nodes[1].node.get_and_clear_pending_msg_events();
			match events.pop().unwrap() {
				MessageSendEvent::UpdateHTLCs { node_id: _, channel_id: _, updates: msgs::CommitmentUpdate { update_fail_htlcs, commitment_signed, .. } } => {
					nodes[0].node.handle_update_fail_htlc(nodes[1].node.get_our_node_id(), &update_fail_htlcs[0]);
					commitment_signed_dance!(nodes[0], nodes[1], commitment_signed, false);
				},
				_ => panic!("Unexpected event"),
...
pub fn test_update_err_monitor_lockdown() {
	...
	nodes[0].node.handle_update_fulfill_htlc(nodes[1].node.get_our_node_id(), &updates.update_fulfill_htlcs[0]);
	{
		...
		if let Some(channel) = get_channel_ref!(nodes[0], nodes[1], node_0_per_peer_lock, node_0_peer_state_lock, chan_1.2).as_funded_mut() {
			assert_eq!(updates.commitment_signed.len(), 1);
			if let Ok(Some(update)) = channel.commitment_signed(&updates.commitment_signed[0], &node_cfgs[0].logger) {
...
fn do_test_onchain_htlc_settlement_after_close(broadcast_alice: bool, go_onchain_before_fulfill: bool) {
	...
	nodes[1].node.handle_commitment_signed_batch_test(nodes[2].node.get_our_node_id(), &carol_updates.commitment_signed);
	...
	check_added_monitors!(nodes[2], 1);
	nodes[2].node.handle_commitment_signed_batch_test(nodes[1].node.get_our_node_id(), &bob_updates.commitment_signed);
	check_added_monitors!(nodes[2], 1);
...
fn do_test_tx_confirmed_skipping_blocks_immediate_broadcast(test_height_before_timelock: bool) {
	...
	*nodes[0].connect_style.borrow_mut() = ConnectStyle::BestBlockFirstSkippingBlocks;
	...
	if test_height_before_timelock {
		...
		expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1], vec![HTLCHandlingType::ForwardFailed { node_id: Some(nodes[2].node.get_our_node_id()), channel_id: channel_id }]);
		check_added_monitors!(nodes[1], 1);
		...
		expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1],
			vec![HTLCHandlingType::ForwardFailed { node_id: Some(node_c_id), channel_id }]);
	}
}
...
fn do_test_dup_htlc_second_rejected(test_for_second_fail_panic: bool) {
	...
	if test_for_second_fail_panic {
		...
		let expected_destinations = vec![
			HTLCHandlingType::ReceiveFailed { payment_hash: our_payment_hash },
			HTLCHandlingType::ReceiveFailed { payment_hash: our_payment_hash },
		];
		...
		expect_pending_htlcs_forwardable_and_htlc_handling_failed_ignore!(nodes[1], vec![HTLCHandlingType::ReceiveFailed { payment_hash: our_payment_hash }]);
		nodes[1].node.process_pending_htlc_forwards();
...
pub fn test_inconsistent_mpp_params() {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed_ignore!(nodes[3], vec![HTLCHandlingType::ReceiveFailed { payment_hash: our_payment_hash }]);
	nodes[3].node.process_pending_htlc_forwards();
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[2], vec![HTLCHandlingType::ForwardFailed { node_id: Some(nodes[3].node.get_our_node_id()), channel_id: chan_2_3.2 }]);
	check_added_monitors!(nodes[2], 1);
...
fn do_test_max_dust_htlc_exposure(dust_outbound_balance: bool, exposure_breach_event: ExposureEvent, on_holder_tx: bool, multiplier_dust_limit: bool, apply_excess_fee: bool) {
	...
	if exposure_breach_event == ExposureEvent::AtHTLCForward {
		...
		expect_pending_htlcs_forwardable!(nodes[0]);
		expect_htlc_handling_failed!(nodes[0].node.get_and_clear_pending_events(), &[HTLCHandlingType::ReceiveFailed { payment_hash }]);
		// With default dust exposure: 5000 sats
		if on_holder_tx {
...
pub fn test_nondust_htlc_excess_fees_are_dust() {
	...
	expect_htlc_handling_failed!(
		...
		&[HTLCHandlingType::ForwardFailed { node_id: Some(node_id_1), channel_id: chan_id_1 }]
	);
...
pub fn test_disconnects_peer_awaiting_response_ticks() {
	...
	*nodes[0].fee_estimator.sat_per_kw.lock().unwrap() *= 2;
	...
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &alice_fee_update.commitment_signed);
	check_added_monitors!(&nodes[1], 1);
	...
	check_added_monitors!(&nodes[0], 1);
	nodes[0].node.handle_commitment_signed_batch_test(nodes[1].node.get_our_node_id(), &bob_commitment_signed);
	check_added_monitors(&nodes[0], 1);
