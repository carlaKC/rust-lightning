	...
	L::Target: Logger,
{
		...
		SP: Fn(SendAlongPathArgs) -> Result<(), APIError>,
	{
		...
		let result = self.pay_route_internal(
			&route, payment_hash, &recipient_onion, keysend_preimage, invoice_request, Some(&bolt12_invoice), None, payment_id,
		...
		SP: Fn(SendAlongPathArgs) -> Result<(), APIError>,
	{
		...
		let res = self.pay_route_internal(&route, payment_hash, &recipient_onion, keysend_preimage,
			invoice_request.as_ref(), bolt12_invoice.as_ref(), trampoline_forward_info.as_ref(), payment_id, Some(total_msat),
		...
		F: Fn(SendAlongPathArgs) -> Result<(), APIError>,
	{
		...
		for (path, session_priv_bytes) in route.paths.iter().zip(onion_session_privs.iter()) {
			let path_res = send_payment_along_path(SendAlongPathArgs {
				...
				hold_htlc_at_next_hop: hold_htlcs_at_next_hop, session_priv_bytes: *session_priv_bytes
			});
