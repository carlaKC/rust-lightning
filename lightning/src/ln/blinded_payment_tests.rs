...
use crate::routing::router::{
	compute_fees, BlindedTail, Path, Payee, PaymentParameters, Route, RouteHop, RouteParameters,
	TrampolineHop,
};
...
fn replacement_onion(
	...
	let (trampoline_packet, outer_total_msat) = {
		...
		if !blinded {
			let eve_trampoline_fees = compute_fees(
				fred_amt_msat,
				RoutingFees {
...
fn do_test_trampoline_relay(blinded: bool, test_case: TrampolineTestCase) {
	...
	*nodes[0].keys_manager.override_random_bytes.lock().unwrap() = Some(override_random_bytes);
	...
	let no_payment_constraints =
		PaymentConstraints { max_cltv_expiry: u32::max_value(), htlc_minimum_msat: fred_recv_amt };
