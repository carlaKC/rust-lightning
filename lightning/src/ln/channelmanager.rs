	...
	L::Target: Logger,
{
	...
	fn fail_htlc_backwards_internal(
		...
		match source {
			...
			HTLCSource::PreviousHopData(HTLCPreviousHopData {
				...
				let failure = get_htlc_forward_failure(
					blinded_failure,
					onion_error,
					incoming_packet_shared_secret,
					trampoline_shared_secret,
			...
			HTLCSource::TrampolineForward {
				...
				for current_hop_data in previous_hop_data {
					...
					let htlc_failure_reason = get_htlc_forward_failure(
						blinded_failure,
						&onion_error,
						incoming_packet_shared_secret,
						&incoming_trampoline_shared_secret,
	...
	fn claim_funds_from_hop_htlc_forward(
	...
	fn claim_funds_internal(
		...
		match source {
			...
			HTLCSource::PreviousHopData(hop_data) => {
				self.claim_funds_from_hop_htlc_forward(
					payment_preimage,
...
fn reconcile_pending_htlcs_with_monitor(
	...
	// what its outcome is. If we still have an entry for this HTLC in `forward_htlcs` or
	// `pending_intercepted_htlcs`, we were apparently not persisted after the monitor was when
	...
	decode_update_add_htlcs.retain(|src_outb_alias, update_add_htlcs| {
		update_add_htlcs.retain(|update_add_htlc| {
			...
			if matches {
				log_info!(
					logger,
					"Removing pending to-decode HTLC as it was forwarded to the closed channel"
				);
			}
			!matches
		});
