		...
		match source {
			...
			HTLCSource::TrampolineForward {
				...
				session_priv: outbound_payment
					.clone()
					.map_or(*incoming_trampoline_shared_secret, |o| o.session_priv.secret_bytes()),
			},
					...
					if mpp_timeout {
						...
						timed_out_mpp_htlcs.push((
							HTLCSource::TrampolineForward {
								...
								incoming_trampoline_shared_secret: payment.htlcs[0]
									.prev_hop
									.incoming_packet_shared_secret,
	...
	> ChannelManager<M, T, ES, NS, SP, F, R, MR, L>
{
	...
	>(
		...
		if let Some(height) = height_opt {
			...
			self.awaiting_trampoline_forwards.lock().unwrap().retain(|payment_hash, payment| {
				if let OnionPayload::Trampoline {  .. } =
					payment.htlcs[0].onion_payload
				{
					...
					if htlc_timed_out {
						...
						timed_out_htlcs.push((
							HTLCSource::TrampolineForward {
								previous_hop_data,
								incoming_trampoline_shared_secret: payment.htlcs[0].prev_hop.incoming_packet_shared_secret,
								outbound_payment: None,
							},
							payment_hash.clone(),
							HTLCFailReason::reason(
...
impl Readable for ClaimableHTLC {
	...
	fn read<R: Read>(reader: &mut R) -> Result<Self, DecodeError> {
		...
		let onion_payload = match (keysend_preimage, trampoline_next_hop) {
			...
			(None, Some(trampoline_next_hop)) => {OnionPayload::Trampoline {
		...
		if let Some(purposes) = claimable_htlc_purposes {
			...
			for (payment_hash, htlcs) in claimable_htlcs_list.into_iter() {
				...
				let purpose = match &htlcs[0].onion_payload {
					...
					OnionPayload::Trampoline { .. } => events::PaymentPurpose::Trampoline {},
				};
				claimable_payments
