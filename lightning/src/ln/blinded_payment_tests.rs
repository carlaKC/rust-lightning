...
fn do_test_unblinded_trampoline_forward(
	failure_scenario: Option<TrampolineForwardFailureScenario>,
) {
	...
	let mut replacement_onion = None;

	// We don't check in failure scenario as the substitution breaks HMAC Legacy verification.
	if failure_scenario.is_none() {
		replacement_onion = {
			...
			nodes[0].node.test_modify_pending_payment(&payment_id, |pmt| {
				if let crate::ln::outbound_payment::PendingOutboundPayment::Retryable {
					session_privs,
					..
				} = pmt
				{
			...
			let outer_session_priv =
				SecretKey::from_slice(&outer_session_priv_hash.to_byte_array()).unwrap();
	...
	if let Some(replacement_onion) = replacement_onion {
		{
	...
	match failure_scenario {
		...
		Some(TrampolineForwardFailureScenario::NoRoute) => {
			...
			.with_payment_preimage(payment_preimage)
			.without_claimable_event()
			.expect_failure(HTLCHandlingFailureType::TrampolineForward {
		...
		Some(TrampolineForwardFailureScenario::InvalidInterTrampolineOnion) => {
			...
			let mut update_message_carol_t0 =
				remove_first_msg_event_to_node(&nodes[4].node.get_our_node_id(), &mut events);
			{
...

			{
				let downstream_id = 4;
				let upstream_id = 2;
				let unblinded_node_updates = get_htlc_update_msgs(
					&nodes[downstream_id],
					&nodes[upstream_id].node.get_our_node_id(),
				);
				nodes[upstream_id].node.handle_update_fail_malformed_htlc(
					nodes[downstream_id].node.get_our_node_id(),
					&unblinded_node_updates.update_fail_malformed_htlcs[0],
				);
				do_commitment_signed_dance(
					&nodes[upstream_id],
					&nodes[downstream_id],
		...
		Some(TrampolineForwardFailureScenario::InvalidRecipientOnion) => {
			...
			let mut update_message_t0_dave =
				remove_first_msg_event_to_node(&nodes[3].node.get_our_node_id(), &mut events);
			{
...
#[test]
fn test_unblinded_trampoline_forward_ok() {
	do_test_unblinded_trampoline_forward(None);
}

#[test]
fn test_unblinded_trampoline_forward_no_route() {
	do_test_unblinded_trampoline_forward(Some(TrampolineForwardFailureScenario::NoRoute));
}

#[test]
fn test_unblinded_trampoline_forward_invalid_inter_trampoline() {
	do_test_unblinded_trampoline_forward(Some(
		TrampolineForwardFailureScenario::InvalidInterTrampolineOnion,
	));
}

#[test]
fn test_unblinded_trampoline_forward_invalid_recipient() {
	do_test_unblinded_trampoline_forward(Some(
		TrampolineForwardFailureScenario::InvalidRecipientOnion,
	));
}
...
fn do_test_blinded_trampoline_forward(failure_scenario: Option<TrampolineForwardFailureScenario>) {
	...
	let alice_carol_trampoline_shared_secret =
		secret_from_hex("a0f4b8d7b6c2d0ffdfaf718f76e9decaef4d9fb38a8c4addb95c4007cc3eee03");
	let carol_blinding_point =
		PublicKey::from_secret_key(&secp_ctx, &alice_carol_trampoline_shared_secret);
	...
	// Blinded path is Carol -> Dave (single hop, Dave is recipient)
	let path = [((dave_node_id, None), WithoutLength(&dave_unblinded_tlvs))];
	let blinded_hops = blinded_path::utils::construct_blinded_hops(
		&secp_ctx,
		path.into_iter(),
		&alice_carol_trampoline_shared_secret,
	);
...
#[test]
fn test_blinded_trampoline_forward_no_route() {
	do_test_blinded_trampoline_forward(Some(TrampolineForwardFailureScenario::NoRoute));
}

#[test]
fn test_blinded_trampoline_forward_invalid_inter_trampoline() {
	do_test_blinded_trampoline_forward(Some(
		TrampolineForwardFailureScenario::InvalidInterTrampolineOnion,
	));
}

#[test]
fn test_blinded_trampoline_forward_invalid_recipient() {
	do_test_blinded_trampoline_forward(Some(
		TrampolineForwardFailureScenario::InvalidRecipientOnion,
	));
}
