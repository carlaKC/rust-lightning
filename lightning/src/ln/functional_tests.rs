...
pub fn test_fee_spike_violation_fails_htlc() {
	...
	expect_pending_htlcs_forwardable!(nodes[1]);
	expect_htlc_handling_failed_destinations!(nodes[1].node.get_and_clear_pending_events(), &[HTLCHandlingType::FailedPayment { payment_hash }]);
...
fn do_test_fail_back_before_backwards_timeout(post_fail_back_action: PostFailBackAction) {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1],
		vec![HTLCHandlingType::NextHopChannel {
...
fn do_test_commitment_revoked_fail_backward_exhaustive(deliver_bs_raa: bool, use_dust: bool, no_to_remote: bool) {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[2], vec![HTLCHandlingType::FailedPayment { payment_hash: first_payment_hash }]);
	check_added_monitors!(nodes[2], 1);
	let updates = get_htlc_update_msgs!(nodes[2], nodes[1].node.get_our_node_id());
	assert!(updates.update_add_htlcs.is_empty());
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[2], vec![HTLCHandlingType::FailedPayment { payment_hash: second_payment_hash }]);
	check_added_monitors!(nodes[2], 1);
	let updates = get_htlc_update_msgs!(nodes[2], nodes[1].node.get_our_node_id());
	assert!(updates.update_add_htlcs.is_empty());
	assert!(updates.update_fulfill_htlcs.is_empty());
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[2], vec![HTLCHandlingType::FailedPayment { payment_hash: third_payment_hash }]);
	check_added_monitors!(nodes[2], 1);
	let updates = get_htlc_update_msgs!(nodes[2], nodes[1].node.get_our_node_id());
	assert!(updates.update_add_htlcs.is_empty());
	assert!(updates.update_fulfill_htlcs.is_empty());
...
fn do_test_fail_backwards_unrevoked_remote_announce(deliver_last_raa: bool, announce_latest: bool) {
	...
	let failed_destinations = vec![
		HTLCHandlingType::FailedPayment { payment_hash: payment_hash_1 },
		HTLCHandlingType::FailedPayment { payment_hash: payment_hash_3 },
		HTLCHandlingType::FailedPayment { payment_hash: payment_hash_5 },
		HTLCHandlingType::FailedPayment { payment_hash: payment_hash_6 },
	];
	...
	let failed_destinations_2 = vec![
		HTLCHandlingType::FailedPayment { payment_hash: payment_hash_2 },
		HTLCHandlingType::FailedPayment { payment_hash: payment_hash_4 },
	];
	...
	let failed_destinations_3 = vec![
		HTLCHandlingType::NextHopChannel { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2 },
		HTLCHandlingType::NextHopChannel { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2 },
		HTLCHandlingType::NextHopChannel { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2 },
		HTLCHandlingType::NextHopChannel { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2 },
		HTLCHandlingType::NextHopChannel { node_id: Some(nodes[5].node.get_our_node_id()), channel_id: chan_3_5.2 },
		HTLCHandlingType::NextHopChannel { node_id: Some(nodes[5].node.get_our_node_id()), channel_id: chan_3_5.2 },
	];
	...
	if deliver_last_raa {
		...
		let expected_destinations: Vec<HTLCHandlingType> = repeat(HTLCHandlingType::NextHopChannel { node_id: Some(nodes[3].node.get_our_node_id()), channel_id: chan_2_3.2 }).take(3).collect();
		...
		let expected_destinations: Vec<HTLCHandlingType> = if announce_latest {
			repeat(HTLCHandlingType::NextHopChannel { node_id: Some(nodes[3].node.get_our_node_id()), channel_id: chan_2_3.2 }).take(9).collect()
		} else {
			repeat(HTLCHandlingType::NextHopChannel { node_id: Some(nodes[3].node.get_our_node_id()), channel_id: chan_2_3.2 }).take(6).collect()
		};
...
fn do_test_failure_delay_dust_htlc_local_commitment(announce_latest: bool) {
	...
	check_added_monitors!(nodes[1], 0);
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1], vec![HTLCHandlingType::FailedPayment { payment_hash: payment_hash_2 }]);
	check_added_monitors!(nodes[1], 1);
...
pub fn test_bump_penalty_txn_on_revoked_htlcs() {
	...
	let events = nodes[0].node.get_and_clear_pending_events();
	expect_pending_htlcs_forwardable_conditions(events[0..2].to_vec(), &[HTLCHandlingType::FailedPayment { payment_hash: failed_payment_hash }]);
...
pub fn test_bump_txn_sanitize_tracking_maps() {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed_ignore!(nodes[0], vec![HTLCHandlingType::FailedPayment { payment_hash: payment_hash_2 }]);
	assert_eq!(nodes[0].tx_broadcaster.txn_broadcasted.lock().unwrap().len(), 0);
...
fn do_test_tx_confirmed_skipping_blocks_immediate_broadcast(test_height_before_timelock: bool) {
	...
	*nodes[0].connect_style.borrow_mut() = ConnectStyle::BestBlockFirstSkippingBlocks;
	...
	if test_height_before_timelock {
		...
		expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1], vec![HTLCHandlingType::NextHopChannel { node_id: Some(nodes[2].node.get_our_node_id()), channel_id: channel_id }]);
		check_added_monitors!(nodes[1], 1);
		...
		expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1],
			vec![HTLCHandlingType::NextHopChannel { node_id: Some(node_c_id), channel_id }]);
	}
}
...
fn do_test_dup_htlc_second_rejected(test_for_second_fail_panic: bool) {
	...
	if test_for_second_fail_panic {
		...
		let expected_destinations = vec![
			HTLCHandlingType::FailedPayment { payment_hash: our_payment_hash },
			HTLCHandlingType::FailedPayment { payment_hash: our_payment_hash },
		];
		...
		expect_pending_htlcs_forwardable_and_htlc_handling_failed_ignore!(nodes[1], vec![HTLCHandlingType::FailedPayment { payment_hash: our_payment_hash }]);
		nodes[1].node.process_pending_htlc_forwards();
...
pub fn test_inconsistent_mpp_params() {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed_ignore!(nodes[3], vec![HTLCHandlingType::FailedPayment { payment_hash: our_payment_hash }]);
	nodes[3].node.process_pending_htlc_forwards();
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[2], vec![HTLCHandlingType::NextHopChannel { node_id: Some(nodes[3].node.get_our_node_id()), channel_id: chan_2_3.2 }]);
	check_added_monitors!(nodes[2], 1);
...
fn do_test_max_dust_htlc_exposure(dust_outbound_balance: bool, exposure_breach_event: ExposureEvent, on_holder_tx: bool, multiplier_dust_limit: bool, apply_excess_fee: bool) {
	...
	if exposure_breach_event == ExposureEvent::AtHTLCForward {
		...
		expect_pending_htlcs_forwardable!(nodes[0]);
		expect_htlc_handling_failed_destinations!(nodes[0].node.get_and_clear_pending_events(), &[HTLCHandlingType::FailedPayment { payment_hash }]);
		// With default dust exposure: 5000 sats
		if on_holder_tx {
...
pub fn test_nondust_htlc_excess_fees_are_dust() {
	...
	expect_htlc_handling_failed_destinations!(
		...
		&[HTLCHandlingType::NextHopChannel { node_id: Some(node_id_1), channel_id: chan_id_1 }]
	);
