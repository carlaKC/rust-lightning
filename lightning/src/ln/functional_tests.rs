...
use crate::events::{Event, FundingInfo, PathFailure, PaymentPurpose, ClosureReason, HTLCDestination, PaymentFailureReason, HTLCDestinationFailure};
...
pub fn test_update_fee_that_funder_cannot_afford() {
	...
	commitment_signed_dance!(nodes[1], nodes[0], update_msg.commitment_signed, None);
...
pub fn holding_cell_htlc_counting() {
	...
	commitment_signed_dance!(nodes[0], nodes[1], bs_fail_updates.commitment_signed, None, true);
...
pub fn test_channel_reserve_holding_cell_htlcs() {
	...
	commitment_signed_dance!(nodes[2], nodes[1], payment_event_11.commitment_msg, None);
	...
	commitment_signed_dance!(nodes[1], nodes[0], &commitment_update_2.commitment_signed, None);
	expect_pending_htlcs_forwardable!(nodes[1]);
	...
	commitment_signed_dance!(nodes[2], nodes[1], &payment_event_3.commitment_msg, None);
	expect_pending_htlcs_forwardable!(nodes[2]);
...
pub fn test_simple_commitment_revoked_fail_backward() {
	...
	match events[0] {
		MessageSendEvent::UpdateHTLCs { ref node_id, updates: msgs::CommitmentUpdate { ref update_add_htlcs, ref update_fail_htlcs, ref update_fulfill_htlcs, ref update_fail_malformed_htlcs, ref commitment_signed, .. } } => {
			...
			nodes[0].node.handle_update_fail_htlc(nodes[1].node.get_our_node_id(), &update_fail_htlcs[0]);
			commitment_signed_dance!(nodes[0], nodes[1], commitment_signed, None, true);
...
fn do_test_commitment_revoked_fail_backward_exhaustive(deliver_bs_raa: bool, use_dust: bool, no_to_remote: bool) {
	...
	let bs_raa = commitment_signed_dance!(nodes[1], nodes[2], updates.commitment_signed, None, true, false, true);
...
fn do_test_holding_cell_htlc_add_timeouts(forwarded_htlc: bool) {
	...
	if forwarded_htlc {
		expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1], vec![HTLCDestination::NextHopChannel {
			node_id: Some(nodes[2].node.get_our_node_id()),
			channel_id: chan_2.2,
			reason: Some(LocalHTLCFailureReason::CLTVExpiryTooSoon.into()),
		}]);
		check_added_monitors!(nodes[1], 1);
		...
		match fail_commit[0] {
			MessageSendEvent::UpdateHTLCs { updates: msgs::CommitmentUpdate { ref update_fail_htlcs, ref commitment_signed, .. }, .. } => {
				nodes[0].node.handle_update_fail_htlc(nodes[1].node.get_our_node_id(), &update_fail_htlcs[0]);
				commitment_signed_dance!(nodes[0], nodes[1], commitment_signed, Some(FailureType::Downstream), true);
			},
			_ => unreachable!(),
		}
...
pub fn test_duplicate_payment_hash_one_failure_one_success() {
	...
	check_added_monitors!(nodes[2], 1);
	commitment_signed_dance!(nodes[2], nodes[4], &updates.commitment_signed, None);
...
fn do_test_fail_backwards_unrevoked_remote_announce(deliver_last_raa: bool, announce_latest: bool) {
	...
	commitment_signed_dance!(nodes[3], nodes[4], four_removes.commitment_signed, None);
	...
	commitment_signed_dance!(nodes[3], nodes[5], two_removes.commitment_signed, None);
	...
	let failed_destinations_3 = vec![
		HTLCDestination::NextHopChannel { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2, reason: Some(HTLCDestinationFailure::Downstream{}) },
		HTLCDestination::NextHopChannel { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2, reason: Some(HTLCDestinationFailure::Downstream{}) },
		HTLCDestination::NextHopChannel { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2, reason: Some(HTLCDestinationFailure::Downstream{}) },
		HTLCDestination::NextHopChannel { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2, reason: Some(HTLCDestinationFailure::Downstream{}) },
		HTLCDestination::NextHopChannel { node_id: Some(nodes[5].node.get_our_node_id()), channel_id: chan_3_5.2, reason: Some(HTLCDestinationFailure::Downstream{}) },
		HTLCDestination::NextHopChannel { node_id: Some(nodes[5].node.get_our_node_id()), channel_id: chan_3_5.2, reason: Some(HTLCDestinationFailure::Downstream{}) },
	];
	...
	if deliver_last_raa {
		commitment_signed_dance!(nodes[2], nodes[3], six_removes.commitment_signed, None);
	} else {
		let _cs_last_raa = commitment_signed_dance!(nodes[2], nodes[3], six_removes.commitment_signed, None, true, false, true);
	}
	...
	if deliver_last_raa {
		...
		let expected_destinations: Vec<HTLCDestination> = repeat(HTLCDestination::NextHopChannel {
		...
		let expected_destinations: Vec<HTLCDestination> = if announce_latest {
			repeat(HTLCDestination::NextHopChannel {
				node_id: Some(nodes[3].node.get_our_node_id()),
				channel_id: chan_2_3.2,
				reason: Some(LocalHTLCFailureReason::ChannelClosed.into()),
			}).take(9).collect()
	...
	for msg in cs_msgs {
		match msg {
			MessageSendEvent::UpdateHTLCs { ref node_id, ref updates } => {
				...
				commitment_signed_dance!(target, nodes[2], updates.commitment_signed, None, true);
			},
			_ => panic!("Unexpected event"),
		}
	}
...
pub fn test_free_and_fail_holding_cell_htlcs() {
	...
	commitment_signed_dance!(nodes[0], nodes[1], update_msgs.commitment_signed, None, true);
	expect_payment_sent!(nodes[0], payment_preimage_1);
}
...
pub fn test_update_fulfill_htlc_bolt2_missing_badonion_bit_for_malformed_htlc_message() {
	...
	nodes[1].node.handle_update_add_htlc(nodes[0].node.get_our_node_id(), &updates.update_add_htlcs[0]);
	check_added_monitors!(nodes[1], 0);
	commitment_signed_dance!(nodes[1], nodes[0], updates.commitment_signed, None, true);
	expect_pending_htlcs_forwardable!(nodes[1]);
...
pub fn test_update_fulfill_htlc_bolt2_after_malformed_htlc_message_must_forward_update_fail_htlc() {
	...
	check_added_monitors!(nodes[1], 0);
	commitment_signed_dance!(nodes[1], nodes[2], update_msg.1, None, true);
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1], vec![HTLCDestination::NextHopChannel {
		node_id: Some(nodes[2].node.get_our_node_id()),
		channel_id: chan_2.2,
		reason: Some(LocalHTLCFailureReason::InvalidOnionVersion.into()),
	}]);
...
pub fn test_channel_failed_after_message_with_badonion_node_perm_bits_set() {
	...
	let mut reason: Option<LocalHTLCFailureReason> = None;
	let events_3 = nodes[2].node.get_and_clear_pending_msg_events();
	assert_eq!(events_3.len(), 1);
	match events_3[0] {
		MessageSendEvent::UpdateHTLCs { ref updates, .. } => {
			...
			reason = Some(update_msg.failure_code.into());

			nodes[1].node.handle_update_fail_malformed_htlc(nodes[2].node.get_our_node_id(), &update_msg);
			commitment_signed_dance!(nodes[1], nodes[2], updates.commitment_signed, None, true);
		},
		_ => panic!("Unexpected event"),
	}
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1],
		vec![HTLCDestination::NextHopChannel {
			node_id: Some(nodes[2].node.get_our_node_id()), channel_id: chan_2.2, reason: Some(reason.unwrap().into()) }]);
	let events_4 = nodes[1].node.get_and_clear_pending_msg_events();
	assert_eq!(events_4.len(), 1);
	check_added_monitors!(nodes[1], 1);
...
fn do_test_dup_htlc_second_rejected(test_for_second_fail_panic: bool) {
	...
	if test_for_second_fail_panic {
		...
		commitment_signed_dance!(nodes[0], nodes[1], fail_updates_1.commitment_signed, None);
...
fn do_test_max_dust_htlc_exposure(dust_outbound_balance: bool, exposure_breach_event: ExposureEvent, on_holder_tx: bool, multiplier_dust_limit: bool, apply_excess_fee: bool) {
	...
	if exposure_breach_event == ExposureEvent::AtHTLCForward {
		...
		let payment_event = SendEvent::from_event(events.remove(0));
		nodes[0].node.handle_update_add_htlc(nodes[1].node.get_our_node_id(), &payment_event.msgs[0]);
		commitment_signed_dance!(nodes[0], nodes[1], payment_event.commitment_msg, None);
		expect_pending_htlcs_forwardable!(nodes[0]);
...
pub fn test_nondust_htlc_excess_fees_are_dust() {
	...
	commitment_signed_dance!(nodes[0], nodes[2], send.commitment_msg, None, true);
	...
	commitment_signed_dance!(nodes[2], nodes[0], fail.commitment_signed, None);
...
fn do_payment_with_custom_min_final_cltv_expiry(valid_delta: bool, use_user_hash: bool) {
	...
	if valid_delta {
		...
		commitment_signed_dance!(nodes[0], nodes[1], fail_updates.commitment_signed, None, true);
...
fn do_test_multi_post_event_actions(do_reload: bool) {
	...
	for dest in &[1, 2] {
		...
		commitment_signed_dance!(nodes[0], nodes[*dest], htlc_fulfill_updates.commitment_signed, None);
		check_added_monitors(&nodes[0], 0);
	}
