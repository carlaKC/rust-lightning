...
fn do_test_trampoline_relay(blinded: bool, test_case: TrampolineTestCase) {
	...
	*nodes[0].keys_manager.override_random_bytes.lock().unwrap() = Some(override_random_bytes);
	...
	let final_cltv_height = fred_cltv_final + starting_htlc_offset + excess_final_cltv + 1;
	...
	let args = if payment_failure.is_some() {
		...
		// Note that we added on one extra block of CLTV budget for Eve that she doesn't need, so
		// we expect our final htlc to arrive with one block to spare on our required final height.
		args.with_payment_secret(payment_secret).with_payment_claimable_cltv(final_cltv_height + 1)
	};
