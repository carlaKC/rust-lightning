...
fn abandoned_send_payment_idempotent() {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1], [HTLCHandlingType::FailedPayment { payment_hash: first_payment_hash }]);
...
fn do_test_custom_tlvs(spontaneous: bool, even_tlvs: bool, known_tlvs: bool) {
	...
	match (known_tlvs, even_tlvs) {
		...
		(false, true) => {
			nodes[1].node.claim_funds(our_payment_preimage);
			let expected_destinations = vec![HTLCHandlingType::FailedPayment { payment_hash: our_payment_hash }];
