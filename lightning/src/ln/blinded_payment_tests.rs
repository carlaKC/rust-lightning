...
fn do_test_unblinded_trampoline_forward(failure_scenario: Option<TrampolineForwardFailureScenario>) {
	...
	if failure_scenario.is_none() {
		replacement_onion = {
			...
			let (outer_payloads, _, _) = onion_utils::build_onion_payloads(&route.paths[0], outer_total_msat, &recipient_onion_fields, outer_starting_htlc_offset, &None, None, Some(trampoline_packet)).unwrap(); let outer_onion_keys = onion_utils::construct_onion_keys(&secp_ctx, &route.clone().paths[0], &outer_session_priv);
			let outer_packet = onion_utils::construct_onion_packet(
				outer_payloads,
				outer_onion_keys,
				prng_seed.secret_bytes(),
	...
	match failure_scenario {
		...
		Some(TrampolineForwardFailureScenario::NoRoute) => {
			...
			let args = PassAlongPathArgs::new(&nodes[0], route, amt_msat, payment_hash, first_message_event)
				.with_payment_preimage(payment_preimage)
				.without_claimable_event()
				.expect_failure(HTLCHandlingFailureType::FailedTrampolineForward { requested_next_node_id: dave_node_id, forward_scid: None });
			do_pass_along_path(args);
		...
		Some(TrampolineForwardFailureScenario::InvalidInterTrampolineOnion) => {
			...
			let args = PassAlongPathArgs::new(&nodes[2], route_carol_t0, amt_msat, payment_hash, update_message_carol_t0.clone()).expect_failure(HTLCHandlingFailureType::InvalidOnion);
			do_pass_along_path(args);
		...
		Some(TrampolineForwardFailureScenario::InvalidRecipientOnion) => {
			...
			let args = PassAlongPathArgs::new(&nodes[4], route_to_dave, amt_msat, payment_hash, update_message_t0_dave.clone()).expect_failure(HTLCHandlingFailureType::InvalidOnion);
			do_pass_along_path(args);
