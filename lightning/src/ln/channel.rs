...
use super::onion_utils::{LocalHTLCFailure, HTLCFailureDetails};
	...
	<SP::Target as SignerProvider>::EcdsaSigner: EcdsaChannelSigner
{
	...
	fn internal_htlc_satisfies_config(
		...
		if fee.is_none() || htlc.amount_msat < fee.unwrap() ||
			...
			return Err(HTLCFailureDetails {
				failure_code: 0x1000 | 12, // fee_insufficient
				failure_reason: None,
				msg: "Prior hop has deviated from specified fees parameters or origin node has obsolete ones",
			});
		}
		if (htlc.cltv_expiry as u64) < outgoing_cltv_value as u64 + config.cltv_expiry_delta as u64 {
			return Err(HTLCFailureDetails {
				failure_code: 0x1000 | 13, // incorrect_cltv_expiry
				failure_reason: None,
				msg: "Forwarding node has tampered with the intended HTLC values or origin node has an obsolete cltv_expiry_delta",
			});
		}
		Ok(())
	}
	...
	{
		if self.context.channel_state.is_local_shutdown_sent() {
			return Err(LocalHTLCFailure::ShutdownSent.into())
		}
		...
		if on_counterparty_tx_dust_htlc_exposure_msat > max_dust_htlc_exposure_msat {
			...
			return Err(LocalHTLCFailure::DustLimitCounterparty.into())
		}
		...
		if msg.amount_msat / 1000 < exposure_dust_limit_success_sats {
			...
			if on_holder_tx_dust_htlc_exposure_msat > max_dust_htlc_exposure_msat {
				...
				return Err(LocalHTLCFailure::DustLimitHolder.into())
			}
		}
		...
		if !self.funding.is_outbound() {
			...
			if pending_remote_value_msat.saturating_sub(self.funding.holder_selected_channel_reserve_satoshis * 1000).saturating_sub(anchor_outputs_value_msat) < remote_fee_cost_incl_stuck_buffer_msat {
				...
				return Err(LocalHTLCFailure::FeeSpikeBuffer.into());
			}
		}
