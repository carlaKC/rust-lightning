...
pub enum PendingHTLCRouting {
	...
	TrampolineForward {
		...
		/// MPP data from the outer onion for accumulating trampoline forward HTLCs.
		multipath_trampoline_data: Option<msgs::FinalOnionHopData>,
	},
	...
	L::Target: Logger,
{
	...
	// claimable_payments, and a result representing the payment claim status. The result will
	// be `Ok(Some(...))` if the payment is complete and ready to forward, `Ok(None)` if
	/// we're still waiting for additional MPP parts, or `Err(...)` if the HTLC should be failed.
	fn check_claimable_incoming_htlc<'a>(
		&self, claimable_payment: &'a mut ClaimablePayment, purpose: events::PaymentPurpose,
		claimable_htlc: ClaimableHTLC, mut onion_fields: RecipientOnionFields,
		payment_hash: PaymentHash,
	) -> (bool, Result<Option<(&'a ClaimablePayment, u64, u32)>, ()>) {
		...
		if total_value >= msgs::MAX_VALUE_MSAT {
			...
			debug_assert!(
				total_value.saturating_sub(amount_msat)
					<= claimable_payment.total_counterparty_skimmed_msat()
			);
			claimable_payment.htlcs.sort();
			(committed_to_claimable, Ok(Some((claimable_payment, amount_msat, earliest_expiry))))
		} else {
			...
			(committed_to_claimable, Ok(None))
		}
	}
				...
				HTLCForwardInfo::AddHTLC(payment) => {
					...
					macro_rules! handle_incoming_htlc {
						...
						$payment_hash: expr) => {{
							...
							match res {
								Ok(Some((claimable_payment, amount_msat, earliest_expiry))) => {
									let payment_id = claimable_payment
										.inbound_payment_id(&self.inbound_payment_id_secret);
									...
									new_events.push_back((
										events::Event::PaymentClaimable {
											...
											claim_deadline: Some(
												earliest_expiry - HTLC_FAIL_BACK_BUFFER,
											),
											onion_fields,
											payment_id: Some(payment_id),
										},
	...
	pub fn timer_tick_occurred(&self) {
		PersistenceNotifierGuard::optionally_notify(self, || {
			...
			self.awaiting_trampoline_forwards.lock().unwrap().retain(|payment_hash, payment| {
				if let OnionPayload::Trampoline { incoming_trampoline_shared_secret, .. } =
					payment.htlcs[0].onion_payload
				{
					let htlc_total_msat: u64 =
						payment.htlcs.iter().map(|h| h.sender_intended_value).sum();
					let mpp_timeout = check_mpp_timeout(htlc_total_msat, &mut payment.htlcs);
					if mpp_timeout {
						let (prev_hops, attempted_htlcs): (Vec<_>, Vec<_>) = payment
							.htlcs
							.drain(..)
							.map(|claimable| {
								let locator = events::HTLCLocator {
									channel_id: claimable.prev_hop.channel_id,
									user_channel_id: claimable.prev_hop.user_channel_id,
									node_id: claimable.prev_hop.counterparty_node_id,
								};
								(claimable.prev_hop, locator)
							})
							.unzip();
		...
		if let Some(height) = height_opt {
			...
			self.awaiting_trampoline_forwards.lock().unwrap().retain(|payment_hash, payment| {
					...
					payment.htlcs[0].onion_payload
				{
					...
					if htlc_timed_out {
						let (previous_hop_data, attempted_htlcs): (Vec<_>, Vec<_>) = payment
							.htlcs
							.drain(..)
							.map(|claimable| {
								let locator = events::HTLCLocator {
...
impl_writeable_tlv_based_enum!(PendingHTLCRouting,
	...
	(3, TrampolineForward) => {
		...
		(10, multipath_trampoline_data, option),
	}
);
	...
	L::Target: Logger,
{
	fn read<Reader: io::Read>(
		...
		// TODO: read pending_trampoline_forwards
