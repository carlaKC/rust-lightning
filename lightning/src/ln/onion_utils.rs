		...
		L::Target: Logger,
	{
		match htlc_source {
			...
			HTLCSource::TrampolineForward { ref outbound_payment, .. } => {
				// TODO: how should we handle this better than an unwrap? we should never try
				// to decode an onion failure if we didn't dispatch HTLCs for the trampoline?
				// TODO: can we avoid this clone?
				let (_, path, session_priv) = outbound_payment.clone().unwrap();
				process_onion_failure_inner(
					secp_ctx,
					logger,
					&path,
		...
		match self.0 {
			...
			HTLCFailReasonRepr::Reason { ref data, ref failure_reason } => {
				...
				match htlc_source {
					...
					&HTLCSource::TrampolineForward { ref outbound_payment, .. } => {
						let short_channel_id = if let Some(outbound_payment) = outbound_payment {
							Some(outbound_payment.1.hops[0].short_channel_id)
						} else {
							None
						};
						decoded_onion_failure!(short_channel_id, *failure_reason, data)
					},
					_ => unreachable!(),
				}
			},
...
/// Builds a payment onion for an inter-trampoline payment.
pub(crate) fn create_trampoline_forward_onion<T: secp256k1::Signing>(
	secp_ctx: &Secp256k1<T>, path: &Path, session_priv: &SecretKey, total_msat: u64,
	payment_secret: PaymentSecret, payment_hash: &PaymentHash,
...
pub(crate) fn create_payment_onion_internal<T: secp256k1::Signing>(
	...
	if let Some(blinded_tail) = &path.blinded_tail {
		if !blinded_tail.trampoline_hops.is_empty() {
			...
			trampoline_packet_option = Some((trampoline_packet, None));
		}
	}
