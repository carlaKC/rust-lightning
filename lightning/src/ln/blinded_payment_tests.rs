...
/// Creates a blinded tail, using Carol as a receiving trampoline using a blinded path with one
/// dummy hop.
fn create_blinded_tail(
	secp_ctx: &Secp256k1<All>, override_random_bytes: [u8; 32], carol_node_id: PublicKey,
	carol_expanded_key: ExpandedKey, trampoline_cltv_expiry_delta: u32, final_value_msat: u64,
	...
	let carol_blinded_hops = {
		...
		.authenticate(nonce, &carol_expanded_key)
		.encode();

		let path = [((carol_node_id, None), WithoutLength(&payee_tlvs))];
...
fn do_test_trampoline_relay(blinded: bool, test_case: TrampolineTestCase) {
	...
	*nodes[0].keys_manager.override_random_bytes.lock().unwrap() = Some(override_random_bytes);
	...
	let route = Route {
		paths: vec![Path {
			...
			blinded_tail: Some(create_blinded_tail(
				...
				override_random_bytes,
				carol_node_id,
				nodes[2].keys_manager.get_expanded_key(),
	...
	if let Some(failure) = payment_failure {
		let unblinded_node_updates = get_htlc_update_msgs!(nodes[2], bob_node_id);
		nodes[1]
			.node
			.handle_update_fail_htlc(carol_node_id, &unblinded_node_updates.update_fail_htlcs[0]);
		...
		let unblinded_node_updates = get_htlc_update_msgs!(nodes[1], alice_node_id);
		nodes[0]
			.node
			.handle_update_fail_htlc(bob_node_id, &unblinded_node_updates.update_fail_htlcs[0]);
		do_commitment_signed_dance(
			&nodes[0],
			&nodes[1],
			&unblinded_node_updates.commitment_signed,
