...
use crate::events::{
	self, ClosureReason, Event, EventHandler, EventsProvider, HTLCDestination,
	HTLCDestinationFailure, InboundChannelFunds, PaymentFailureReason, ReplayEvent,
};
...
use crate::ln::onion_payment::{
	...
	decode_incoming_update_add_htlc_onion, HopConnector, InboundHTLCErr, NextPacketDetails,
};
use crate::ln::onion_utils;
use crate::ln::onion_utils::{
	HTLCFailReason, HTLCFailureDetails, LocalHTLCFailureReason, INVALID_ONION_BLINDING,
};
#[cfg(test)]
	...
	L::Target: Logger,
{
	...
	fn list_funded_channels_with_filter<
		Fn: FnMut(&(&ChannelId, &FundedChannel<SP>)) -> bool + Copy,
	>(
		&self, f: Fn,
	) -> Vec<ChannelDetails> {
	...
	pub fn list_channels(&self) -> Vec<ChannelDetails> {
		...
		let mut res = Vec::with_capacity(self.short_to_chan_info.read().unwrap().len());
		{
			...
			for (_cp_id, peer_state_mutex) in per_peer_state.iter() {
				let mut peer_state_lock = peer_state_mutex.lock().unwrap();
				let peer_state = &mut *peer_state_lock;
				for (context, funding) in peer_state
					.channel_by_id
					.iter()
	...
	fn can_forward_htlc_to_outgoing_channel(
		...
		if let HopConnector::ShortChannelId(outgoing_scid) = next_packet.outgoing_connector {
			...
			return Err((
				...
				LocalHTLCFailureReason::NodeIDForward,
			));
		}
		...
		if next_packet.outgoing_amt_msat < chan.context.get_counterparty_htlc_minimum_msat() {
			return Err((
				...
				LocalHTLCFailureReason::HTLCBelowMinimum,
			));
		}
	...
	fn can_forward_htlc(
		...
		let outgoing_scid = match next_packet_details.outgoing_connector {
			...
			HopConnector::Trampoline(_) => {
				return Err((
					"Cannot forward by Node ID without SCID.",
					LocalHTLCFailureReason::NodeIDForward.into(),
				));
			},
		};
		match self.do_funded_channel_callback(outgoing_scid, |chan: &mut FundedChannel<SP>| {
			...
			Some(Ok(())) => {},
			Some(Err(e)) => return Err((e.0, e.1.into())),
			None => {
	...
	fn htlc_failure_from_update_add_err(
		...
		reason: HTLCFailureDetails, is_intro_node_blinded_forward: bool, shared_secret: &[u8; 32],
	) -> HTLCFailureMsg {
		...
		HTLCFailureMsg::Relay(msgs::UpdateFailHTLC {
			channel_id: msg.channel_id,
			htlc_id: msg.htlc_id,
			reason: HTLCFailReason::reason(reason.failure_code(), err_data.to_vec())
	...
	fn construct_pending_htlc_fail_msg<'a>(
		...
		return HTLCFailureMsg::Relay(msgs::UpdateFailHTLC {
			channel_id: msg.channel_id,
			htlc_id: msg.htlc_id,
			reason: HTLCFailReason::reason(
				inbound_err.reason.failure_code(),
				inbound_err.err_data.to_vec(),
			)
			.get_encrypted_failure_packet(&shared_secret, &None),
		});
	}
			...
			reply_paths
				...
				.flat_map(|reply_path| {
					invoice
						.message_paths()
						.iter()
						.map(move |invoice_path| (invoice_path, reply_path))
				})
	...
	pub fn fail_intercepted_htlc(&self, intercept_id: InterceptId) -> Result<(), APIError> {
			...
			payment.forward_info.routing
		{
			...
			let reason = HTLCFailReason::from_failure_code(0x4000 | 10);
			let destination =
				...
				match self.get_pending_htlc_info(
					...
					Err(inbound_err) => {
						let htlc_destination = get_failed_htlc_destination(
							outgoing_scid_opt,
							update_add_htlc.payment_hash,
							inbound_err.reason.clone(),
						);
						htlc_fails.push((
							self.construct_pending_htlc_fail_msg(
								&update_add_htlc,
								&incoming_counterparty_node_id,
								shared_secret,
								inbound_err,
										...
										macro_rules! failure_handler {
											($msg: expr, $reason: expr, $err_data: expr, $phantom_ss: expr, $next_hop_unknown: expr) => {
												...
												failed_forwards.push((htlc_source, payment_hash,
													HTLCFailReason::reason($reason.failure_code(), $err_data),
													reason
												));
												continue;
											}
						...
						let queue_fail_htlc_res = match forward_info {
							HTLCForwardInfo::AddHTLC(PendingAddHTLCInfo {
								...
								let maybe_optimal_channel = peer_state
									...
									.filter_map(|chan| {
										let balances = chan.context.get_available_balances(
											&chan.funding,
											&self.fee_estimator,
										);
								...
								if let Err(e) = optimal_channel.queue_add_htlc(
										...
										.and_then(Channel::as_funded_mut)
									{
										...
										let data = self.get_htlc_inbound_temp_fail_data(
	...
	fn fail_holding_cell_htlcs(
		...
		let (failure_reason, onion_failure_data) = {
			...
			if let Some(peer_state_mutex) = per_peer_state.get(counterparty_node_id) {
				...
				match peer_state.channel_by_id.entry(channel_id) {
					...
					hash_map::Entry::Vacant(_) => {
						(LocalHTLCFailureReason::UnknownNextPeer.into(), Vec::new())
					},
				}
			} else {
				(LocalHTLCFailureReason::UnknownNextPeer.into(), Vec::new())
		...
		for (htlc_src, payment_hash) in htlcs_to_fail.drain(..) {
			let reason =
				HTLCFailReason::reason(failure_reason.failure_code(), onion_failure_data.clone());
			let receiver = HTLCDestination::NextHopChannel {
				node_id: Some(counterparty_node_id.clone()),
				channel_id,
				reason: Some(HTLCDestinationFailure::Local {
					reason: failure_reason.clone().into(),
				}),
			};
									...
									} = &blocker
									{
										if *pending_claim == claim_ptr {
											...
											pending_claim_state.channels_without_preimage.retain(
												|htlc_info| {
													let this_claim = htlc_info.counterparty_node_id
														== counterparty_node_id
											...
											if pending_claim_state
												...
												.is_empty()
											{
												for htlc_info in pending_claim_state
													.channels_with_preimage
													.iter()
												{
													let freed_chan = (
		...
		let unblock_chan = |chan: &mut Channel<SP>,
		 ...
		 -> Option<ShutdownResult> {
			...
			if let Some(msgs) = chan.signer_maybe_unblocked(self.chain_hash, &&logger) {
				...
				let cu_msg = msgs
					.commitment_update
					.map(|updates| MessageSendEvent::UpdateHTLCs { node_id, updates });
				let raa_msg = msgs
					.revoke_and_ack
