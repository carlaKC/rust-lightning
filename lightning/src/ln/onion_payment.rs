...
use crate::ln::onion_utils::{HTLCFailReason, INVALID_ONION_BLINDING, ONION_DATA_LEN, HTLCFailureDetails, LocalHTLCFailureReason};
...
pub struct InboundHTLCErr {
	...
	pub err_data: Vec<u8>,
	/// Reason that the HTLC failed.
	pub reason: HTLCFailureDetails,
	/// Error message text.
	pub msg: &'static str,
}
		...
		onion_utils::Hop::BlindedForward { next_hop_data: msgs::InboundOnionBlindedForwardPayload {
			...
			let (amt_to_forward, outgoing_cltv_value) = check_blinded_forward(
				...
				InboundHTLCErr {
					...
					reason: INVALID_ONION_BLINDING.into(),
					err_data: vec![0; 32],
				}
			})?;
...
pub(super) fn create_recv_pending_htlc_info(
	...
	if cltv_expiry <= current_height + HTLC_FAIL_BACK_BUFFER + 1 {
		...
		return Err(InboundHTLCErr {
			reason: (0x4000 | 15).into(), err_data,
	...
	{
		return Err(InboundHTLCErr {
			reason: LocalHTLCFailureReason::IncorrectHTLCAmount.into(),
	...
	let routing = if let Some(payment_preimage) = keysend_preimage {
		...
		if hashed_preimage != payment_hash {
			return Err(InboundHTLCErr {
				reason: LocalHTLCFailureReason::InvalidKeysendPreimage.into(),
				err_data: Vec::new(),
		...
		return Err(InboundHTLCErr {
			reason: LocalHTLCFailureReason::PaymentSecretRequired.into(),
			err_data: Vec::new(),
	...
	L::Target: Logger,
{
	let (hop, next_packet_details_opt) =
		...
		InboundHTLCErr { msg, reason: err_code.into(), err_data }
	})?;
	Ok(match hop {
	...
	L::Target: Logger,
{
	...
	let next_hop = match onion_utils::decode_next_payment_hop(
		...
		Err(onion_utils::OnionDecodeErr::Relay { err_msg, err_code, shared_secret, trampoline_shared_secret }) => {
			return encode_relay_error(err_msg, err_code.into(), shared_secret.secret_bytes(), trampoline_shared_secret.map(|tss| tss.secret_bytes()), &[0; 0]);
		},
	};
			...
			let (amt_to_forward, outgoing_cltv_value) = match check_blinded_forward(
				...
				Err(()) => {
					return encode_relay_error("Underflow calculating outbound amount or cltv value for blinded forward",
						INVALID_ONION_BLINDING.into(), shared_secret.secret_bytes(), None, &[0; 32]);
				}
			};
			let next_packet_pubkey = onion_utils::next_hop_pubkey(&secp_ctx,
				msg.onion_routing_packet.public_key.unwrap(), &shared_secret.secret_bytes());
...
pub(super) fn check_incoming_htlc_cltv(
	...
	if cltv_expiry <= cur_height + HTLC_FAIL_BACK_BUFFER as u32 {
		return Err(("CLTV expiry is too close", LocalHTLCFailureReason::CLTVExpiryTooClose));
	}
	if cltv_expiry > cur_height + CLTV_FAR_FAR_AWAY as u32 {
		return Err(("CLTV expiry is too far in the future", LocalHTLCFailureReason::CLTVExpiryTooClose));
	}
	...
	if (outgoing_cltv_value) as u64 <= (cur_height + LATENCY_GRACE_PERIOD_BLOCKS) as u64 {
		return Err(("Outgoing CLTV value is too soon", LocalHTLCFailureReason::CLTVExpiryTooClose));
	}
