...
use crate::blinded_path::payment::{
	...
	ForwardNode, ForwardTlvs, PaymentConstraints, PaymentContext, PaymentForwardNode, PaymentRelay,
	ReceiveTlvs, TrampolineForwardTlvs, PAYMENT_PADDING_ROUND_OFF,
};
...
fn replacement_onion(
	...
	// because LDK doesn't support unblinded trampoline receives.
...
fn do_test_trampoline_relay(blinded: bool, test_case: TrampolineTestCase) {
	...
	*nodes[0].keys_manager.override_random_bytes.lock().unwrap() = Some(override_random_bytes);
	...
	let no_payment_constraints = PaymentConstraints {
		max_cltv_expiry: u32::max_value(),
		htlc_minimum_msat: fred_recv_amt,
	};
	let intermediate_nodes = [
		ForwardNode {
			tlvs: TrampolineForwardTlvs {
				next_trampoline: eve_node_id,
				payment_relay: carol_relay.clone(),
		...
		ForwardNode {
			tlvs: TrampolineForwardTlvs {
				...
				payment_relay: eve_trampoline_relay.clone(),
				payment_constraints: no_payment_constraints.clone(),
				features: BlindedHopFeatures::empty(),
				next_blinding_override: None,
			},
	...
	// Compute the total fees for the blinded path to report to the sender.
	let (base_msat, proportional_millionths) = compute_aggregated_base_prop_fee(
		[&carol_relay, &eve_trampoline_relay].iter().map(|relay| RoutingFees {
	...
	let blinded_tail = create_trampoline_forward_blinded_tail(
		&secp_ctx,
		&nodes[5].keys_manager,
		&intermediate_nodes,
		fred_node_id,
		...
		vec![TrampolineHop {
			...
			node_features: Features::empty(),
			fee_msat: total_fees,
			cltv_expiry_delta: carol_relay.cltv_expiry_delta as u32
				+ eve_trampoline_relay.cltv_expiry_delta as u32
				+ fred_cltv_final
				+ excess_final_cltv,
		}],
		excess_final_cltv,
...
///
/// Carol's inner trampoline onion is a forward to a dummy next trampoline. We don't need the
