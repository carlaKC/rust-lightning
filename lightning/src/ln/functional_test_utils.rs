...
pub fn do_commitment_signed_dance(node_a: &Node<'_, '_, '_>, node_b: &Node<'_, '_, '_>, commitment_signed: &msgs::CommitmentSigned, fail_backwards: bool, skip_last_step: bool) {
	...
	if fail_backwards {
		expect_pending_htlcs_forwardable_and_htlc_handling_failed!(node_a,
			vec![crate::events::HTLCDestination::NextHopChannel{ node_id: Some(node_b.node.get_our_node_id()), channel_id: commitment_signed.channel_id, reason: None }]);
		check_added_monitors!(node_a, 1);
	...
	for (i, (expected_route, (path_msgs, next_hop))) in expected_paths.iter().zip(per_path_msgs.drain(..)).enumerate() {
		...
		for (idx, node) in expected_route.iter().rev().enumerate().skip(1) {
			...
			if next_msgs.is_some() {
				...
				if !update_next_node {
					expect_pending_htlcs_forwardable_and_htlc_handling_failed!(node, vec![HTLCDestination::NextHopChannel { node_id: Some(prev_node.node.get_our_node_id()), channel_id: next_msgs.as_ref().unwrap().0.channel_id, reason: None }]);
				}
			}
			let events = node.node.get_and_clear_pending_msg_events();
			if update_next_node {
