...
fn test_monitor_update_fail_no_rebroadcast() {
	...
	let send_event = SendEvent::from_event(nodes[0].node.get_and_clear_pending_msg_events().remove(0));
	nodes[1].node.handle_update_add_htlc(nodes[0].node.get_our_node_id(), &send_event.msgs[0]);
	let bs_raa = commitment_signed_dance!(nodes[1], nodes[0], send_event.commitment_msg, None, true, false, true);
...
fn do_test_monitor_update_fail_raa(test_ignore_second_cs: bool) {
	...
	let bs_revoke_and_ack = commitment_signed_dance!(nodes[1], nodes[2], updates.commitment_signed, None, true, false, true);
	check_added_monitors!(nodes[0], 0);
...
fn monitor_update_claim_fail_no_response() {
	...
	let payment_event = SendEvent::from_event(events.pop().unwrap());
	nodes[1].node.handle_update_add_htlc(nodes[0].node.get_our_node_id(), &payment_event.msgs[0]);
	let as_raa = commitment_signed_dance!(nodes[1], nodes[0], payment_event.commitment_msg, None, true, false, true);
...
fn test_fail_htlc_on_broadcast_after_claim() {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1],
		vec![HTLCDestination::NextHopChannel { node_id: Some(nodes[2].node.get_our_node_id()),
			channel_id: chan_id_2, reason: Some(LocalHTLCFailureReason::ChannelClosed.into()) }]);
...
fn do_channel_holding_cell_serialize(disconnect: bool, reload_a: bool) {
	...
	commitment_signed_dance!(nodes[1], nodes[0], (), None, true, false, false);
...
fn test_blocked_chan_preimage_release() {
	...
	check_added_monitors(&nodes[1], 1);
	let (a, raa) = do_main_commitment_signed_dance(&nodes[1], &nodes[0], None);
	assert!(a.is_none());
