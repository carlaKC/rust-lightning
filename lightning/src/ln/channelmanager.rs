...
macro_rules! handle_monitor_update_completion {
	($self: ident, $peer_state_lock: expr, $peer_state: expr, $per_peer_state_lock: expr, $chan: expr) => { {
		...
		for failure in updates.failed_htlcs.drain(..) {
			let receiver = HTLCDestination::NextHopChannel { node_id: Some(counterparty_node_id), channel_id, reason: Some(failure.2.into()) };
	...
	fn can_forward_htlc_to_outgoing_channel(
		...
		if let HopConnector::ShortChannelId(outgoing_scid) = next_packet.outgoing_connector {
			...
			return Err(("Cannot forward by Node ID without SCID.",
				LocalHTLCFailureReason::InvalidTrampolineRoute));
		}
	...
	fn fail_holding_cell_htlcs(
		...
		for (htlc_src, payment_hash) in htlcs_to_fail.drain(..) {
			...
			let receiver = HTLCDestination::NextHopChannel {
				...
				channel_id,
				reason: Some(failure_reason.clone().into()),
			};
