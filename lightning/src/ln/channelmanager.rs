...
use crate::ln::onion_utils::{HTLCFailReason, INVALID_ONION_BLINDING, HTLCFailureDetails, LocalHTLCFailureReason};
...
impl Into<HTLCFailureDetails> for FailureCode {
	fn into(self) -> HTLCFailureDetails {
		match self {
			FailureCode::TemporaryNodeFailure => (0x2000 | 2).into(),
			FailureCode::RequiredNodeFeatureMissing => (0x4000 | 0x2000 | 3).into(),
			FailureCode::IncorrectOrUnknownPaymentDetails => (0x4000 | 15).into(),
			FailureCode::InvalidOnionPayload(_) => (0x4000 | 22).into(),
		}
	}
}
	...
	L::Target: Logger,
{
	...
	fn close_channel_internal(&self, channel_id: &ChannelId, counterparty_node_id: &PublicKey, target_feerate_sats_per_1000_weight: Option<u32>, override_shutdown_script: Option<ShutdownScript>) -> Result<(), APIError> {
		...
		for htlc_source in failed_htlcs.drain(..) {
			...
			let reason = HTLCFailReason::from_failure_details(details.into());
	...
	fn can_forward_htlc_to_outgoing_channel(
		...
		if !chan.context.is_live() {
			if !chan.context.is_enabled() {
				return Err((
					...
					LocalHTLCFailureReason::ChannelDisabled.into(),
				));
			} else {
				return Err((
		...
		if next_packet.outgoing_amt_msat < chan.context.get_counterparty_htlc_minimum_msat() {
			return Err((
				...
				LocalHTLCFailureReason::HTLCBelowMinimum.into(),
			));
		}
	...
	fn can_forward_htlc(
		...
		match self.do_funded_channel_callback(next_packet_details.outgoing_scid, |chan: &mut FundedChannel<SP>| {
			...
			None => {
				...
				{} else {
					return Err(("Don't have available channel for forwarding as requested.", (0x4000 | 10).into()));
				}
			}
		}
	...
	fn htlc_failure_from_update_add_err(
		...
		reason: HTLCFailureDetails, is_intro_node_blinded_forward: bool,
		shared_secret: &[u8; 32]
	) -> HTLCFailureMsg {
		// at capacity, we write fields `htlc_msat` and `len`
		let mut res = VecWriter(Vec::with_capacity(8 + 2));
		let err_code = reason.failure_code();
	...
	fn get_pending_htlc_info<'a>(
		...
		match decoded_hop {
			onion_utils::Hop::Receive { .. } | onion_utils::Hop::BlindedReceive { .. } => {
				...
				create_recv_pending_htlc_info(inbound_onion_payload, shared_secret, msg.payment_hash,
					msg.amount_msat, msg.cltv_expiry, None, allow_underpay, msg.skimmed_fee_msat,
					current_height)
			},
			onion_utils::Hop::Forward { next_hop_data, next_hop_hmac, new_packet_bytes, .. } => {
				create_fwd_pending_htlc_info(msg, msgs::InboundOnionPayload::Forward(next_hop_data), next_hop_hmac,
					new_packet_bytes, shared_secret, next_packet_pubkey_opt)
			},
			onion_utils::Hop::BlindedForward { next_hop_data, next_hop_hmac, new_packet_bytes, .. } => {
				create_fwd_pending_htlc_info(msg, msgs::InboundOnionPayload::BlindedForward(next_hop_data), next_hop_hmac,
					new_packet_bytes, shared_secret, next_packet_pubkey_opt) 			}
		}
	}
	...
	pub fn fail_intercepted_htlc(&self, intercept_id: InterceptId) -> Result<(), APIError> {
		...
		if let PendingHTLCRouting::Forward { short_channel_id, incoming_cltv_expiry, .. } = payment.forward_info.routing {
			...
			let reason = HTLCFailReason::from_failure_details((0x4000 | 10).into());
				...
				match self.short_to_chan_info.read().unwrap().get(&outgoing_scid) {
					Some((outgoing_counterparty_node_id, outgoing_channel_id)) =>
						HTLCDestination::NextHopChannel {
							...
							reason: Some(HTLCDestinationFailure::Local { reason: reason.failure_reason() }),
						},
											...
											if phantom_pubkey_res.is_ok() && fake_scid::is_valid_phantom(&self.fake_scid_rand_bytes, short_chan_id, &self.chain_hash) {
												let next_hop = match onion_utils::decode_next_payment_hop(
													...
													Err(onion_utils::OnionDecodeErr::Malformed { err_msg, err_code }) => {
														...
														let reason: HTLCFailureDetails = err_code.into();
												...
												let reason: HTLCFailureDetails = (0x4000 | 10).into();
												fail_forward!(format!("Unknown short channel id {} for forward HTLC", short_chan_id), reason, Vec::new(), None);
											}
										} else {
											let reason: HTLCFailureDetails = (0x4000 | 10).into();
											fail_forward!(format!("Unknown short channel id {} for forward HTLC", short_chan_id), reason, Vec::new(), None);
							...
							HTLCForwardInfo::AddHTLC(PendingAddHTLCInfo {
										...
										&&logger)
								{
										...
										.and_then(Channel::as_funded_mut)
									{
										...
										failed_forwards.push((htlc_source, payment_hash,
											HTLCFailReason::reason(details.into(), data),
								...
								macro_rules! fail_htlc {
									($htlc: expr, $payment_hash: expr) => {
										...
										failed_forwards.push((HTLCSource::PreviousHopData(HTLCPreviousHopData {
											...
											HTLCFailReason::reason((0x4000 | 15).into(), htlc_msat_height_data),
	...
	pub fn timer_tick_occurred(&self) {
		PersistenceNotifierGuard::optionally_notify(self, || {
			...
			for htlc_source in timed_out_mpp_htlcs.drain(..) {
				let details = LocalHTLCFailureReason::MPPTimeout;
	...
	fn get_htlc_fail_reason_from_failure_code(&self, failure_code: FailureCode, htlc: &ClaimableHTLC) -> HTLCFailReason {
		match failure_code {
			FailureCode::TemporaryNodeFailure => HTLCFailReason::from_failure_details(failure_code.into()),
			FailureCode::RequiredNodeFeatureMissing => HTLCFailReason::from_failure_details(failure_code.into()),
	...
	fn fail_holding_cell_htlcs(
				...
				match peer_state.channel_by_id.entry(channel_id) {
					hash_map::Entry::Occupied(chan_entry) => {
						if let Some(_chan) = chan_entry.get().as_funded() {
							let reason: HTLCFailureDetails = (0x1000|7).into();
							let data = self.get_htlc_inbound_temp_fail_data(reason.failure_code());
							(reason, data)
						} else {
		...
		for (htlc_src, payment_hash) in htlcs_to_fail.drain(..) {
			...
			let receiver = HTLCDestination::NextHopChannel { node_id: Some(counterparty_node_id.clone()), channel_id, reason: Some(failure_reason.into()) };
			...
			HTLCSource::PreviousHopData(HTLCPreviousHopData {
				...
				let failure = match blinded_failure {
					Some(BlindedFailure::FromIntroductionNode) => {
						let blinded_onion_error = HTLCFailReason::reason(INVALID_ONION_BLINDING.into(), vec![0; 32]);
	...
	fn claim_payment_internal(&self, payment_preimage: PaymentPreimage, custom_tlvs_known: bool) {
		...
		if valid_mpp {
			...
			for htlc in sources {
				...
				htlc_msat_height_data.extend_from_slice(&self.best_block.read().unwrap().height.to_be_bytes());
				let source = HTLCSource::PreviousHopData(htlc.prev_hop);
				let reason = HTLCFailReason::reason((0x4000 | 15).into(), htlc_msat_height_data);
				let receiver = HTLCDestination::FailedPayment { payment_hash };
				self.fail_htlc_backwards_internal(&source, &payment_hash, &reason, receiver);
			}
			self.claimable_payments.lock().unwrap().pending_claiming_payments.remove(&payment_hash);
								...
								match pending_intercepts.entry(intercept_id) {
									...
									hash_map::Entry::Occupied(_) => {
										...
										failed_intercept_forwards.push((htlc_source, forward_info.payment_hash,
												HTLCFailReason::from_failure_details(LocalHTLCFailureReason::DuplicateIntercept.into()),
						...
						Some(funded_channel) => {
							...
							if let Ok((channel_ready_opt, mut timed_out_pending_htlcs, announcement_sigs)) = res {
								for (source, payment_hash) in timed_out_pending_htlcs.drain(..) {
									...
									let data = self.get_htlc_inbound_temp_fail_data(details.failure_code());
									timed_out_htlcs.push((source, payment_hash, HTLCFailReason::reason(details.into(), data),
				...
				payment.htlcs.retain(|htlc| {
					...
					if height >= htlc.cltv_expiry - HTLC_FAIL_BACK_BUFFER {
						...
						timed_out_htlcs.push((HTLCSource::PreviousHopData(htlc.prev_hop.clone()), payment_hash.clone(),
							HTLCFailReason::reason(details.into(), htlc_msat_height_data),
			...
			intercepted_htlcs.retain(|_, htlc| {
				if height >= htlc.forward_info.outgoing_cltv_value - HTLC_FAIL_BACK_BUFFER {
					...
					timed_out_htlcs.push((prev_hop_data, htlc.forward_info.payment_hash,
							HTLCFailReason::from_failure_details(LocalHTLCFailureReason::InterceptTimeout.into()),
	...
	fn reject_excessively_underpaying_htlcs() {
		...
		if let Err(crate::ln::channelmanager::InboundHTLCErr { reason, .. }) =
			create_recv_pending_htlc_info(hop_data, [0; 32], PaymentHash([0; 32]),
				...
				current_height)
		{
			assert_eq!(reason.failure_code(), 19);
		} else { panic!(); }
