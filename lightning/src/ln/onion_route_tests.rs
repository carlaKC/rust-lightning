				...
				F3: FnMut(),
{
	...
	let update_1_0 = match test_case {
		...
		1|2|3|200 => { // final node failure; forwarding to 2
			...
			commitment_signed_dance!(nodes[2], nodes[1], update_1.commitment_signed, None, true);
			...
			commitment_signed_dance!(nodes[1], nodes[2], update_2_1.commitment_signed, Some(FailureType::Downstream));
	...
	commitment_signed_dance!(nodes[0], nodes[1], update_1_0.commitment_signed, None, true);
	...
	if let &Event::PaymentPathFailed { ref payment_failed_permanently, ref short_channel_id, ref error_code, failure: PathFailure::OnPath { ref network_update }, .. } = &events[0] {
		...
		if let Some(expected_reason) = expected_error_reason {
			assert_eq!(expected_reason.failure_code(), error_code.unwrap())
		}
...
fn test_onion_failure() {
	...
	// intermediate node failure
	let failure = LocalHTLCFailureReason::TemporaryNodeFailure;
	run_onion_failure_test_with_fail_intercept("temporary_node_failure", 100, &nodes, &route, &payment_hash, &payment_secret, |msg| {
		...
		let session_priv = SecretKey::from_slice(&[3; 32]).unwrap();
		let onion_keys = onion_utils::construct_onion_keys(&Secp256k1::new(), &route.paths[0], &session_priv).unwrap();
		msg.reason = onion_utils::build_first_hop_failure_packet(onion_keys[0].shared_secret.as_ref(), failure, &[0;0]);
	}, ||{}, true, Some(failure), Some(NetworkUpdate::NodeFailure{node_id: route.paths[0].hops[0].pubkey, is_permanent: false}), Some(route.paths[0].hops[0].short_channel_id),
		Some(next_hop_failure!(LocalHTLCFailureReason::FeeInsufficient))); // real failure surfaced in event
	...
	run_onion_failure_test_with_fail_intercept("temporary_node_failure", 200, &nodes, &route, &payment_hash, &payment_secret, |_msg| {}, |msg| {
		...
		let session_priv = SecretKey::from_slice(&[3; 32]).unwrap();
		let onion_keys = onion_utils::construct_onion_keys(&Secp256k1::new(), &route.paths[0], &session_priv).unwrap();
		msg.reason = onion_utils::build_first_hop_failure_packet(onion_keys[1].shared_secret.as_ref(), failure, &[0;0]);
	}, ||{
		nodes[2].node.fail_htlc_backwards(&payment_hash);
	}, true, Some(failure), Some(NetworkUpdate::NodeFailure{node_id: route.paths[0].hops[1].pubkey, is_permanent: false}), Some(route.paths[0].hops[1].short_channel_id), None);
	let (_, payment_hash, payment_secret) = get_payment_preimage_hash!(nodes[2]);

	// intermediate node failure
	let failure = LocalHTLCFailureReason::PermanentNodeFailure;
	run_onion_failure_test_with_fail_intercept("permanent_node_failure", 100, &nodes, &route, &payment_hash, &payment_secret, |msg| {
		...
		let onion_keys = onion_utils::construct_onion_keys(&Secp256k1::new(), &route.paths[0], &session_priv).unwrap();
		msg.reason = onion_utils::build_first_hop_failure_packet(onion_keys[0].shared_secret.as_ref(), failure, &[0;0]);
	}, ||{}, true, Some(failure), Some(NetworkUpdate::NodeFailure{node_id: route.paths[0].hops[0].pubkey, is_permanent: true}), Some(route.paths[0].hops[0].short_channel_id),
		Some(next_hop_failure!(LocalHTLCFailureReason::FeeInsufficient))); // real failure surfaced in event
	...
	run_onion_failure_test_with_fail_intercept("permanent_node_failure", 200, &nodes, &route, &payment_hash, &payment_secret, |_msg| {}, |msg| {
		...
		nodes[2].node.fail_htlc_backwards(&payment_hash);
	}, false, Some(failure), Some(NetworkUpdate::NodeFailure{node_id: route.paths[0].hops[1].pubkey, is_permanent: true}), Some(route.paths[0].hops[1].short_channel_id), None);
	let (_, payment_hash, payment_secret) = get_payment_preimage_hash!(nodes[2]);

	// intermediate node failure
	let failure = LocalHTLCFailureReason::RequiredNodeFeature;
	...
	let failure = LocalHTLCFailureReason::TemporaryChannelFailure;
	run_onion_failure_test_with_fail_intercept("temporary_channel_failure", 100, &nodes, &route, &payment_hash, &payment_secret, |msg| {
		msg.amount_msat -= 1;
	}, |msg| {
		let session_priv = SecretKey::from_slice(&[3; 32]).unwrap();
		let onion_keys = onion_utils::construct_onion_keys(&Secp256k1::new(), &route.paths[0], &session_priv).unwrap();
		msg.reason = onion_utils::build_first_hop_failure_packet(onion_keys[0].shared_secret.as_ref(), failure, &err_data);
	}, ||{}, true, Some(failure),
	Some(NetworkUpdate::ChannelFailure { short_channel_id, is_permanent: false }),
	Some(short_channel_id), Some(next_hop_failure!(LocalHTLCFailureReason::FeeInsufficient))); // real failure surfaced in event
	...
	run_onion_failure_test_with_fail_intercept("temporary_channel_failure", 100, &nodes, &route, &payment_hash, &payment_secret, |msg| {
		...
		let session_priv = SecretKey::from_slice(&[3; 32]).unwrap();
		let onion_keys = onion_utils::construct_onion_keys(&Secp256k1::new(), &route.paths[0], &session_priv).unwrap();
		msg.reason = onion_utils::build_first_hop_failure_packet(onion_keys[0].shared_secret.as_ref(), failure, &err_data_without_type);
	}, ||{}, true, Some(failure),
	Some(NetworkUpdate::ChannelFailure { short_channel_id, is_permanent: false }),
	Some(short_channel_id), Some(next_hop_failure!(LocalHTLCFailureReason::FeeInsufficient))); // real failure surfaced in event

	let short_channel_id = channels[1].0.contents.short_channel_id;
	let failure = LocalHTLCFailureReason::PermanentChannelFailure;
	...
	}, ||{}, true, Some(failure), Some(NetworkUpdate::ChannelFailure{short_channel_id, is_permanent: true}), Some(short_channel_id),
		Some(next_hop_failure!(LocalHTLCFailureReason::FeeInsufficient))); // real failure surfaced in event

	let short_channel_id = channels[1].0.contents.short_channel_id;
	let failure = LocalHTLCFailureReason::RequiredChannelFeature;
	...
	let failure = LocalHTLCFailureReason::AmountBelowMinimum;
	run_onion_failure_test("amount_below_minimum", 100, &nodes, &bogus_route, &payment_hash, &payment_secret, |_| {}, ||{}, true, Some(failure),
		Some(NetworkUpdate::ChannelFailure { short_channel_id, is_permanent: false }),
		Some(short_channel_id), Some(next_hop_failure!(failure)));
	...
	let short_channel_id = channels[1].0.contents.short_channel_id;
	let failure = LocalHTLCFailureReason::FeeInsufficient;
	run_onion_failure_test("fee_insufficient", 100, &nodes, &route, &payment_hash, &payment_secret, |msg| {
		msg.amount_msat -= 1;
	}, || {}, true, Some(failure), Some(NetworkUpdate::ChannelFailure { short_channel_id, is_permanent: false}), Some(short_channel_id),
		Some(next_hop_failure!(failure)));

	let short_channel_id = channels[1].0.contents.short_channel_id;
	let failure = LocalHTLCFailureReason::IncorrectCLTVExpiry;
	...
	let short_channel_id = channels[1].0.contents.short_channel_id;
	let failure = LocalHTLCFailureReason::CLTVExpiryTooSoon;
	...
	let failure = LocalHTLCFailureReason::ChannelDisabled;
	run_onion_failure_test("channel_disabled", 100, &nodes, &route, &payment_hash, &payment_secret, |_| {}, || {
		// disconnect event to the channel between nodes[1] ~ nodes[2]
	...
	let failure = LocalHTLCFailureReason::CLTVExpiryTooFar;
	...
	let failure = LocalHTLCFailureReason::MPPTimeout;
	run_onion_failure_test_with_fail_intercept("mpp_timeout", 200, &nodes, &route, &payment_hash, &payment_secret, |_msg| {}, |msg| {
		...
		nodes[2].node.fail_htlc_backwards(&payment_hash);
	}, true, Some(failure), None, None, None);
	...
	run_onion_failure_test_with_fail_intercept("0-length channel update in intermediate node UPDATE onion failure",
		...
		Some(channels[1].0.contents.short_channel_id), Some(next_hop_failure!(LocalHTLCFailureReason::FeeInsufficient)));
