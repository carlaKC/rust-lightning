...
use crate::events::{Event, ClosureReason, HTLCHandlingType, HTLCHandlingFailureReason};
use crate::ln::{channel, LocalHTLCFailureReason};
...
fn test_no_preimage_inbound_htlc_balances() {
	...
	expect_pending_htlcs_forwardable_conditions!(nodes[0],
		[(
		HTLCHandlingType::ReceiveFailed { payment_hash: to_a_failed_payment_hash },
			HTLCHandlingFailureReason::from(LocalHTLCFailureReason::InvalidOnionPayload),
	)]);
...
fn do_test_revoked_counterparty_commitment_balances(anchors: bool, confirm_htlc_spend_first: bool) {
	...
	events.retain(|ev| {
		match ev {
			Event::HTLCHandlingFailed { handling_type: HTLCHandlingType::ForwardFailed { node_id, channel_id }, .. } => {
				assert_eq!(*channel_id, chan_id);
				assert_eq!(*node_id, Some(nodes[1].node.get_our_node_id()));
				false
			},
			Event::HTLCHandlingFailed { handling_type: HTLCHandlingType::ReceiveFailed { payment_hash }, .. } => {
...
fn do_test_revoked_counterparty_htlc_tx_balances(anchors: bool) {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed_ignore!(&nodes[0],
		[(
		HTLCHandlingType::ReceiveFailed { payment_hash: failed_payment_hash },
			HTLCHandlingFailureReason::from(LocalHTLCFailureReason::InvalidOnionPayload),
	)]);
