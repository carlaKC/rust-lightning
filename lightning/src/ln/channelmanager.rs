...
pub(super) struct BlindedForward {
	next_hop_blinding_point: PublicKey,
	...
	L::Target: Logger,
{
	...
	fn construct_fwd_pending_htlc_info(
		...
		let (short_channel_id, amt_to_forward, outgoing_cltv_value, we_are_intro_node)
			= match hop_data
		{
			msgs::InboundPayload::Forward { short_channel_id, amt_to_forward, outgoing_cltv_value } =>
				(short_channel_id, amt_to_forward, outgoing_cltv_value, false),
			msgs::InboundPayload::BlindedForward {
				...
				(short_channel_id, amt_to_forward, outgoing_cltv_value, intro_node_blinding_point.is_some())
			},
		...
		Ok(PendingHTLCInfo {
			routing: PendingHTLCRouting::Forward {
				...
				short_channel_id,
				blinded: next_blinding_pt.map(|bp| BlindedForward {
					next_hop_blinding_point: bp,
					we_are_intro_node,
				}),
			},
	...
	fn construct_recv_pending_htlc_info(
		...
		let (
			...
			msgs::InboundPayload::Receive {
				...
				(payment_data, keysend_preimage, amt_msat, outgoing_cltv_value, payment_metadata, None),
			msgs::InboundPayload::BlindedReceive {
				...
				(Some(payment_data), None, amt_msat, outgoing_cltv_value, None, blinded)
			}
		...
		let (outgoing_scid, outgoing_amt_msat, outgoing_cltv_value, next_pks_opt) = match next_hop {
			...
			onion_utils::Hop::Forward {
					...
					&self.secp_ctx, curr_blinding_point, &encrypted_tlvs_ss)
				{
					Ok(p) => p,
					Err(_) => {
						return_blinded_htlc_err!("Failed to compute next hop blinding point");
					},
				};
				(short_channel_id, amt_to_forward, outgoing_cltv_value,
								...
								match forward_info {
									HTLCForwardInfo::AddHTLC(PendingAddHTLCInfo {
										...
										if let Err(e) = chan.get_mut().queue_add_htlc(outgoing_amt_msat, payment_hash,
											...
											blinded.map(|b| b.next_hop_blinding_point), &self.fee_estimator,
											&self.logger)
										{
									...
									HTLCForwardInfo::FailMalformedHTLC { htlc_id } => {
										...
										if let Err(e) = chan.get_mut().queue_fail_blinded_intermed_htlc( htlc_id, &self.logger) {
											if let ChannelError::Ignore(msg) = e {
												log_trace!(self.logger, "Failed to fail malformed HTLC with ID {} backwards to short_id {}: {}", htlc_id, short_chan_id, msg);
											} else {
												panic!("Stated return value requirements in queue_fail_blinded_intermed_htlc() were not met");
											}
											// fail-backs are best-effort, we probably already have one
											// pending, and if not that's OK, if not, the channel is on
											// the chain and sending the HTLC-Timeout is their problem.
							...
							HTLCForwardInfo::AddHTLC(PendingAddHTLCInfo {
								...
								let (
									...
									PendingHTLCRouting::ReceiveKeysend {
										payment_data, payment_preimage, payment_metadata, incoming_cltv_expiry, blinded,
									} => {
			...
			HTLCSource::PreviousHopData(HTLCPreviousHopData {
				...
				let fail_forward_info = match blinded {
					Some(true) => {
						...
						log_trace!(self.logger, "Failing blinded HTLC with payment hash {} backwards from us with {:?}, original onion error {:?}", log_bytes!(payment_hash.0), blinded_onion_error, onion_error);
						HTLCForwardInfo::FailHTLC { htlc_id: *htlc_id, err_packet }
					},
					Some(false) => {
						log_trace!(self.logger, "Failing blinded HTLC with payment hash {} backwards from us with malformed, original onion error {:?}", log_bytes!(payment_hash.0), onion_error);
						HTLCForwardInfo::FailMalformedHTLC { htlc_id: *htlc_id }
					},
					None => {
...
impl_writeable_tlv_based!(BlindedForward, {
	(0, next_hop_blinding_point, required),
...
impl_writeable_tlv_based_enum!(HTLCForwardInfo,
	...
	// TODO: putting this at the top breaks the ser macros, why?
	(0, AddHTLC)
);
