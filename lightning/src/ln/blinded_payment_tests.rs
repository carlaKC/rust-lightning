...
fn route_blinding_spec_test_vector() {
	...
	let (bob_onion, _, _) = onion_utils::create_payment_onion(&secp_ctx, &path, &session_priv, &RecipientOnionFields::spontaneous_empty(amt_msat), cur_height, &PaymentHash([0; 32]), &None, None, [0; 32], false).unwrap();
...
fn test_combined_trampoline_onion_creation_vectors() {
	...
	let (bob_onion, htlc_msat, htlc_cltv) = onion_utils::create_payment_onion_internal(&secp_ctx, &path, &outer_session_key, &recipient_onion_fields, cur_height, &associated_data, &None, None, outer_onion_prng_seed, Some(session_priv), Some([0; 32]), false).unwrap();
...
fn test_trampoline_inbound_payment_decoding() {
	...
	let recipient_onion_fields = RecipientOnionFields::secret_only(payment_secret, amt_msat);
	let (bob_onion, _, _) = onion_utils::create_payment_onion(&secp_ctx, &path, &session_priv, &recipient_onion_fields, cur_height, &PaymentHash([0; 32]), &None, None, [0; 32], false).unwrap();
