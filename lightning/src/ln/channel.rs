	...
	fn internal_htlc_satisfies_config(
		...
		if fee.is_none() || htlc.amount_msat < fee.unwrap() ||
			...
			return Err((
				...
				HTLCFailureDetails::new(0x1000 | 12, LocalFailureReason::InsufficientFee), // fee_insufficient
			));
		}
		...
		L::Target: Logger
	{
		if self.context.channel_state.is_local_shutdown_sent() {
			return Err((
				...
				HTLCFailureDetails::new(0x4000|8, LocalFailureReason::ShutdownSent,
			)))
		}
		...
		if on_counterparty_tx_dust_htlc_exposure_msat > max_dust_htlc_exposure_msat {
			...
			return Err((
				...
				HTLCFailureDetails::new(0x1000|7, LocalFailureReason::DustLimiReached),
			))
		}
		...
		if !self.funding.is_outbound() {
			...
			if pending_remote_value_msat.saturating_sub(self.funding.holder_selected_channel_reserve_satoshis * 1000).saturating_sub(anchor_outputs_value_msat) < remote_fee_cost_incl_stuck_buffer_msat {
				...
				return Err((
					...
					HTLCFailureDetails::new(0x1000|7, LocalFailureReason::FeeSpikeBuffer)
				));
			}
		}
