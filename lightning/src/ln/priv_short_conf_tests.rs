...
use crate::events::{ClosureReason, Event, HTLCHandlingType, HTLCHandlingFailureReason};
...
fn test_priv_forwarding_rejection() {
	...
	expect_htlc_handling_failed!(
		nodes[1].node.get_and_clear_pending_events(), &[(
			...
			HTLCHandlingFailureReason::from(LocalHTLCFailureReason::PrivateChannelForward),
	)]);
