...
use crate::events::{Event, FundingInfo, PathFailure, PaymentPurpose, ClosureReason, HTLCHandlingType, PaymentFailureReason, HTLCHandlingFailureReason};
...
pub fn test_fee_spike_violation_fails_htlc() {
	...
	expect_htlc_handling_failed!(nodes[1].node.get_and_clear_pending_events(), &[(
		...
		HTLCHandlingFailureReason::from(LocalHTLCFailureReason::FeeSpikeBuffer),
	)]);
...
fn do_test_commitment_revoked_fail_backward_exhaustive(deliver_bs_raa: bool, use_dust: bool, no_to_remote: bool) {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[2], vec![(
		HTLCHandlingType::ReceiveFailed { payment_hash: first_payment_hash },
		HTLCHandlingFailureReason::from(LocalHTLCFailureReason::InvalidOnionPayload),
	)]);
	check_added_monitors!(nodes[2], 1);
	let updates = get_htlc_update_msgs!(nodes[2], nodes[1].node.get_our_node_id());
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[2], vec![(
		HTLCHandlingType::ReceiveFailed { payment_hash: second_payment_hash },
		HTLCHandlingFailureReason::from(LocalHTLCFailureReason::InvalidOnionPayload),
	)]);
	check_added_monitors!(nodes[2], 1);
	let updates = get_htlc_update_msgs!(nodes[2], nodes[1].node.get_our_node_id());
...
fn do_test_fail_backwards_unrevoked_remote_announce(deliver_last_raa: bool, announce_latest: bool) {
	...
	let failed_destinations = vec![
		(HTLCHandlingType::ReceiveFailed { payment_hash: payment_hash_1 }, HTLCHandlingFailureReason::Downstream),
		(HTLCHandlingType::ReceiveFailed { payment_hash: payment_hash_3 }, HTLCHandlingFailureReason::Downstream),
		(HTLCHandlingType::ReceiveFailed { payment_hash: payment_hash_5 }, HTLCHandlingFailureReason::Downstream),
		(HTLCHandlingType::ReceiveFailed { payment_hash: payment_hash_6 }, HTLCHandlingFailureReason::Downstream),
	];
	...
	let failed_destinations_3 = vec![
		(HTLCHandlingType::ForwardFailed { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2 }, HTLCHandlingFailureReason::Downstream),
		(HTLCHandlingType::ForwardFailed { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2 }, HTLCHandlingFailureReason::Downstream),
		(HTLCHandlingType::ForwardFailed { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2 }, HTLCHandlingFailureReason::Downstream),
		(HTLCHandlingType::ForwardFailed { node_id: Some(nodes[4].node.get_our_node_id()), channel_id: chan_3_4.2 }, HTLCHandlingFailureReason::Downstream),
		(HTLCHandlingType::ForwardFailed { node_id: Some(nodes[5].node.get_our_node_id()), channel_id: chan_3_5.2 }, HTLCHandlingFailureReason::Downstream),
		(HTLCHandlingType::ForwardFailed { node_id: Some(nodes[5].node.get_our_node_id()), channel_id: chan_3_5.2 }, HTLCHandlingFailureReason::Downstream),
	];
	...
	if deliver_last_raa {
		...
		let expected_destinations: Vec<(HTLCHandlingType, HTLCHandlingFailureReason)> = repeat((
		...
		let expected_destinations: Vec<HTLCHandlingType> = if announce_latest {
			repeat((HTLCHandlingType::ForwardFailed { node_id: Some(nodes[3].node.get_our_node_id()), channel_id: chan_2_3.2 }, HTLCHandlingFailureReason::Downstream)).take(9).collect()
		} else {
			repeat((HTLCHandlingType::ForwardFailed { node_id: Some(nodes[3].node.get_our_node_id()), channel_id: chan_2_3.2 }, HTLCHandlingFailureReason::Downstream)).take(6).collect()
		};
...
pub fn test_update_fulfill_htlc_bolt2_after_malformed_htlc_message_must_forward_update_fail_htlc() {
	...
	expect_pending_htlcs_forwardable!(nodes[2]);
	expect_htlc_handling_failed!(nodes[2].node.get_and_clear_pending_events(), &[(
		HTLCHandlingType::InvalidOnion,
		HTLCHandlingFailureReason::from(LocalHTLCFailureReason::InvalidOnionPayload),
	)]);
	check_added_monitors(&nodes[2], 1);
...
pub fn test_bump_txn_sanitize_tracking_maps() {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed_ignore!(nodes[0], vec![(
		HTLCHandlingType::ReceiveFailed { payment_hash: payment_hash_2 },
		HTLCHandlingFailureReason::from(LocalHTLCFailureReason::InvalidOnionPayload),
	)]);
	assert_eq!(nodes[0].tx_broadcaster.txn_broadcasted.lock().unwrap().len(), 0);
...
fn do_test_tx_confirmed_skipping_blocks_immediate_broadcast(test_height_before_timelock: bool) {
	...
	*nodes[0].connect_style.borrow_mut() = ConnectStyle::BestBlockFirstSkippingBlocks;
	...
	if test_height_before_timelock {
		...
		expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1], vec![(
			HTLCHandlingType::ForwardFailed { node_id: Some(nodes[2].node.get_our_node_id()), channel_id: channel_id },
			HTLCHandlingFailureReason::from(LocalHTLCFailureReason::InvalidOnionPayload),
		)]);
		check_added_monitors!(nodes[1], 1);
		...
		expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1],
			vec![(
				HTLCHandlingType::ForwardFailed { node_id: Some(node_c_id), channel_id },
				HTLCHandlingFailureReason::from(LocalHTLCFailureReason::InvalidOnionPayload),
			)]);
	}
}
...
fn do_test_dup_htlc_second_rejected(test_for_second_fail_panic: bool) {
	...
	if test_for_second_fail_panic {
		...
		let expected_destinations = vec![
			(HTLCHandlingType::ReceiveFailed { payment_hash: our_payment_hash }, HTLCHandlingFailureReason::Downstream),
			(HTLCHandlingType::ReceiveFailed { payment_hash: our_payment_hash }, HTLCHandlingFailureReason::Downstream),
		];
		...
		expect_pending_htlcs_forwardable_and_htlc_handling_failed_ignore!(nodes[1], vec![(
				HTLCHandlingType::ReceiveFailed { payment_hash: our_payment_hash },
				HTLCHandlingFailureReason::from(LocalHTLCFailureReason::InvalidOnionPayload),
		)]);
		nodes[1].node.process_pending_htlc_forwards();
...
pub fn test_inconsistent_mpp_params() {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed_ignore!(nodes[3], vec![(
		HTLCHandlingType::ReceiveFailed { payment_hash: our_payment_hash },
		HTLCHandlingFailureReason::from(LocalHTLCFailureReason::InvalidOnionPayload),
	)]);
	nodes[3].node.process_pending_htlc_forwards();
...
pub fn test_double_partial_claim() {
	...
	let failed_destinations = vec![
		(HTLCHandlingType::ReceiveFailed { payment_hash }, HTLCHandlingFailureReason::Downstream),
		(HTLCHandlingType::ReceiveFailed { payment_hash }, HTLCHandlingFailureReason::Downstream),
	];
...
fn do_test_max_dust_htlc_exposure(dust_outbound_balance: bool, exposure_breach_event: ExposureEvent, on_holder_tx: bool, multiplier_dust_limit: bool, apply_excess_fee: bool) {
	...
	if exposure_breach_event == ExposureEvent::AtHTLCForward {
		...
		expect_pending_htlcs_forwardable!(nodes[0]);
		expect_htlc_handling_failed!(nodes[0].node.get_and_clear_pending_events(), &[(
			HTLCHandlingType::ReceiveFailed { payment_hash },
			HTLCHandlingFailureReason::from(LocalHTLCFailureReason::PaymentSecretRequired),
		)]);
		// With default dust exposure: 5000 sats
		if on_holder_tx {
...
pub fn test_nondust_htlc_excess_fees_are_dust() {
	...
	expect_htlc_handling_failed!(nodes[0].node.get_and_clear_pending_events(), &[(
		HTLCHandlingType::ReceiveFailed { payment_hash },
		HTLCHandlingFailureReason::from(LocalHTLCFailureReason::DustLimitCounterparty),
	)]);
	nodes[0].logger.assert_log("lightning::ln::channel",
		format!("Cannot accept value that would put our total dust exposure at {} over the limit {} on counterparty commitment tx",
			2535000, 2530000), 1);
