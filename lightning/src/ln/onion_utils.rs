...
pub(super) fn build_first_hop_failure_packet(
	shared_secret: &[u8], reason: LocalHTLCFailureReason, failure_data: &[u8],
) -> msgs::OnionErrorPacket {
	let failure_packet = build_failure_packet(shared_secret, reason.failure_code(), failure_data);
	...
	pub(super) fn get_encrypted_failure_packet(
		...
		match self.0 {
			HTLCFailReasonRepr::Reason { ref data, ref reason } => {
				let failure_code = reason.failure_code();
