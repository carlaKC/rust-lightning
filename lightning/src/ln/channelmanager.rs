...
use crate::events::{FundingInfo, HTLCHandlingFailureDetails, HTLCHandlingType};
...
use crate::events::{self, Event, EventHandler, EventsProvider, InboundChannelFunds, ClosureReason, HTLCHandlingFailedReason, PaymentFailureReason, ReplayEvent};
...
macro_rules! handle_monitor_update_completion {
	($self: ident, $peer_state_lock: expr, $peer_state: expr, $per_peer_state_lock: expr, $chan: expr) => { {
		...
		for failure in updates.failed_htlcs.drain(..) {
			let failure_details = HTLCHandlingFailureDetails {
				htlc_type: HTLCHandlingType::Forward { node_id: Some(counterparty_node_id), channel_id },
				reason: HTLCHandlingFailedReason::Downstream,
			};
			$self.fail_htlc_backwards_internal(&failure.0, &failure.1, &failure.2, failure_details);
		}
	} }
}
	...
	L::Target: Logger,
{
	...
	fn close_channel_internal(&self, channel_id: &ChannelId, counterparty_node_id: &PublicKey, target_feerate_sats_per_1000_weight: Option<u32>, override_shutdown_script: Option<ShutdownScript>) -> Result<(), APIError> {
		...
		for htlc_source in failed_htlcs.drain(..) {
			...
			let failure_details = HTLCHandlingFailureDetails {
				htlc_type: HTLCHandlingType::Forward { node_id: Some(*counterparty_node_id), channel_id: *channel_id },
				reason: failure_reason.into(),
			};
			self.fail_htlc_backwards_internal(&htlc_source.0, &htlc_source.1, &reason, failure_details);
		}
	...
	fn finish_close_channel(&self, mut shutdown_res: ShutdownResult) {
		...
		for htlc_source in shutdown_res.dropped_outbound_htlcs.drain(..) {
			...
			self.fail_htlc_backwards_internal(&source, &payment_hash, &reason, failure_details);
		}
	...
	pub fn fail_intercepted_htlc(&self, intercept_id: InterceptId) -> Result<(), APIError> {
		...
		if let PendingHTLCRouting::Forward { short_channel_id, incoming_cltv_expiry, .. } = payment.forward_info.routing {
			...
			let failure_reason = LocalHTLCFailureReason::UnknownNextPeer; //  requested_forward_scid: short_channel_id
			let reason = HTLCFailReason::from_failure_code(failure_reason);
			let failure_details = HTLCHandlingFailureDetails {
				htlc_type: HTLCHandlingType::Invalid,
				reason: failure_reason.into(),
			};
			self.fail_htlc_backwards_internal(&htlc_source, &payment.forward_info.payment_hash, &reason, failure_details);
	...
	pub(crate) fn process_pending_update_add_htlcs(&self) {
			...
			if let Some(outgoing_scid) = outgoing_scid_opt {
				match self.short_to_chan_info.read().unwrap().get(&outgoing_scid) {
					Some((outgoing_counterparty_node_id, outgoing_channel_id)) =>
						HTLCHandlingFailureDetails {
							htlc_type: HTLCHandlingType::Forward {
								node_id: Some(*outgoing_counterparty_node_id),
								channel_id: *outgoing_channel_id,
							},
							reason: reason.into(),
						},
					None => HTLCHandlingFailureDetails {
						htlc_type: HTLCHandlingType::Invalid,
						reason: reason.into(),
					}
				}
				...
				HTLCHandlingFailureDetails {
					htlc_type: HTLCHandlingType::Receive{ payment_hash },
					reason: reason.into(),
				}
			}
		};
		...
		'outer_loop: for (incoming_scid, update_add_htlcs) in decode_update_add_htlcs {
			...
			for update_add_htlc in &update_add_htlcs {
				let (next_hop, next_packet_details_opt) = match decode_incoming_update_add_htlc_onion(
					...
					Err((htlc_fail, reason)) => {
						htlc_fails.push((htlc_fail, HTLCHandlingFailureDetails{
							htlc_type: HTLCHandlingType::Invalid,
							reason: reason.into(),
						}));
						continue;
			...
			for (htlc_fail, failure_details) in htlc_fails.drain(..) {
				...
				self.pending_events.lock().unwrap().push_back((events::Event::HTLCHandlingFailed {
					...
					htlc_type: failure_details.htlc_type,
					reason: Some(failure_details.reason),
				}, None));
			}
		}
	}
								...
								match forward_info {
									HTLCForwardInfo::AddHTLC(PendingAddHTLCInfo {
										...
										macro_rules! failure_handler {
											($msg: expr, $reason: expr, $err_data: expr, $phantom_ss: expr, $failure_details: expr) => {
										...
										macro_rules! fail_unknown_next_peer {
											($msg: expr, $err_data: expr, $phantom_ss: expr) => {
												{
													let reason = LocalHTLCFailureReason::UnknownNextPeer; //  requested_forward_scid: short_chan_id
													failure_handler!($msg, reason, $err_data, $phantom_ss, HTLCHandlingFailureDetails {
														htlc_type: HTLCHandlingType::Invalid,
														reason: reason.into(),
													});
												}
										...
										if let PendingHTLCRouting::Forward { ref onion_packet, .. } = routing {
											...
											if phantom_pubkey_res.is_ok() && fake_scid::is_valid_phantom(&self.fake_scid_rand_bytes, short_chan_id, &self.chain_hash) {
												...
												fail_unknown_next_peer!(format!("Unknown short channel id {} for forward HTLC", short_chan_id),
												Vec::new(), None);
											}
										} else {
											fail_unknown_next_peer!(format!("Unknown short channel id {} for forward HTLC", short_chan_id),
					...
					while let Some(forward_info) = draining_pending_forwards.next() {
						let queue_fail_htlc_res = match forward_info {
							HTLCForwardInfo::AddHTLC(PendingAddHTLCInfo {
								...
								if let Err((_, msg)) = optimal_channel.queue_add_htlc(outgoing_amt_msat,
										...
										&&logger)
								{
										...
										.and_then(Channel::as_funded_mut)
									{
										...
										failed_forwards.push((htlc_source, payment_hash,
											...
											HTLCHandlingFailureDetails {
												htlc_type: HTLCHandlingType::Forward { node_id: Some(chan.context.get_counterparty_node_id()), channel_id: forward_chan_id },
												reason: reason.into(),
											},
										));
									} else {
							...
							HTLCForwardInfo::AddHTLC(PendingAddHTLCInfo {
								...
								macro_rules! fail_htlc {
									($htlc: expr, $payment_hash: expr) => {
										...
										let failure_reason = LocalHTLCFailureReason::IncorrectPaymentDetails;
										failed_forwards.push((HTLCSource::PreviousHopData(HTLCPreviousHopData {
											...
											HTLCFailReason::reason(failure_reason, htlc_msat_height_data),
											HTLCHandlingFailureDetails {
												htlc_type: HTLCHandlingType::Receive{ payment_hash: $payment_hash },
												reason: failure_reason.into(),
											},
										));
	...
	pub fn timer_tick_occurred(&self) {
		PersistenceNotifierGuard::optionally_notify(self, || {
			...
			for htlc_source in timed_out_mpp_htlcs.drain(..) {
				...
				let reason = HTLCFailReason::from_failure_code(failure_reason);
				let failure_details = HTLCHandlingFailureDetails {
					htlc_type: HTLCHandlingType::Receive { payment_hash: htlc_source.1 },
					reason: failure_reason.into(),
				};
				self.fail_htlc_backwards_internal(&source, &htlc_source.1, &reason, failure_details);
			}
	...
	pub fn fail_htlc_backwards_with_reason(&self, payment_hash: &PaymentHash, failure_code: FailureCode) {
		...
		if let Some(payment) = removed_source {
			for htlc in payment.htlcs {
				...
				let failure_details = HTLCHandlingFailureDetails {
					htlc_type: HTLCHandlingType::Receive{ payment_hash: *payment_hash },
					reason: LocalHTLCFailureReason::into(FailureCode::into(failure_code)),
				};
				self.fail_htlc_backwards_internal(&source, &payment_hash, &reason, failure_details);
			}
		}
	...
	fn fail_holding_cell_htlcs(
		...
		for (htlc_src, payment_hash) in htlcs_to_fail.drain(..) {
			...
			let failure_details = HTLCHandlingFailureDetails {
				htlc_type: HTLCHandlingType::Forward { node_id: Some(counterparty_node_id.clone()), channel_id },
				reason: failure_reason.into(),
			};
			self.fail_htlc_backwards_internal(&htlc_src, &payment_hash, &reason, failure_details);
		}
	}

	fn fail_htlc_backwards_internal(&self, source: &HTLCSource, payment_hash: &PaymentHash, onion_error: &HTLCFailReason, destination: HTLCHandlingFailureDetails) {
	...
	fn fail_htlc_backwards_internal_without_forward_event(&self, source: &HTLCSource, payment_hash: &PaymentHash, onion_error: &HTLCFailReason, failure_details: HTLCHandlingFailureDetails) -> bool {
	...
	fn claim_payment_internal(&self, payment_preimage: PaymentPreimage, custom_tlvs_known: bool) {
		...
		let (sources, claiming_payment) = {
			...
			match res {
				...
				Err(htlcs) => {
					for htlc in htlcs {
						let failure_reason = FailureCode::InvalidOnionPayload(None);
						let reason = self.get_htlc_fail_reason_from_failure_code(failure_reason, &htlc);
						let source = HTLCSource::PreviousHopData(htlc.prev_hop);
						let failure_details = HTLCHandlingFailureDetails {
							htlc_type: HTLCHandlingType::Receive { payment_hash },
							reason: LocalHTLCFailureReason::into(FailureCode::into(failure_reason)),
						};
						self.fail_htlc_backwards_internal(&source, &payment_hash, &reason, failure_details);
					}
					return;
		...
		if valid_mpp {
			...
			for htlc in sources {
				...
				let source = HTLCSource::PreviousHopData(htlc.prev_hop);
				let failure_reason = LocalHTLCFailureReason::IncorrectPaymentDetails;
				let reason = HTLCFailReason::reason(failure_reason, htlc_msat_height_data);
				let failure_details = HTLCHandlingFailureDetails {
					htlc_type: HTLCHandlingType::Receive { payment_hash },
					reason: failure_reason.into(),
				};
...
This indicates a bug inside LDK. Please report this error at https://github.com/lightningdevkit/rust-lightning/issues/new";
	...
	fn internal_shutdown(&self, counterparty_node_id: &PublicKey, msg: &msgs::Shutdown) -> Result<(), MsgHandleErrInternal> {
		...
		for htlc_source in dropped_htlcs.drain(..) {
			let reason = LocalHTLCFailureReason::DroppedPending;
			let failure_details = HTLCHandlingFailureDetails {
				htlc_type: HTLCHandlingType::Forward{ node_id: Some(counterparty_node_id.clone()), channel_id: msg.channel_id },
				reason: reason.into(),
			};
			let reason = HTLCFailReason::from_failure_code(reason);
			self.fail_htlc_backwards_internal(&htlc_source.0, &htlc_source.1, &reason, failure_details);
		}
	...
	fn forward_htlcs_without_forward_event(&self, per_source_pending_forwards: &mut [(u64, Option<PublicKey>, OutPoint, ChannelId, u128, Vec<(PendingHTLCInfo, u64)>)]) -> bool {
		...
		for &mut (prev_short_channel_id, prev_counterparty_node_id, prev_funding_outpoint, prev_channel_id, prev_user_channel_id, ref mut pending_forwards) in per_source_pending_forwards {
							   ...
							   fake_scid::is_valid_intercept(&self.fake_scid_rand_bytes, scid, &self.chain_hash)
							{
								...
								match pending_intercepts.entry(intercept_id) {
									...
									hash_map::Entry::Occupied(_) => {
										...
										let reason = LocalHTLCFailureReason::UnknownNextPeer;
										failed_intercept_forwards.push((htlc_source, forward_info.payment_hash,
												HTLCFailReason::from_failure_code(reason),
												HTLCHandlingFailureDetails {
													htlc_type: HTLCHandlingType::Invalid,
													reason: reason.into(),
												},
			...
			for (htlc_source, payment_hash, failure_reason, failure_details) in failed_intercept_forwards.drain(..) {
				push_forward_event |= self.fail_htlc_backwards_internal_without_forward_event(&htlc_source, &payment_hash, &failure_reason, failure_details);
			}
				...
				match monitor_event {
					MonitorEvent::HTLCEvent(htlc_update) => {
						...
						if let Some(preimage) = htlc_update.payment_preimage {
							...
							let failure_details = HTLCHandlingFailureDetails {
								htlc_type: HTLCHandlingType::Forward {
									node_id: Some(counterparty_node_id), channel_id,
								},
								reason: failure_reason.into(),
							};
							let reason = HTLCFailReason::from_failure_code(failure_reason);
							self.fail_htlc_backwards_internal(&htlc_update.source, &htlc_update.payment_hash, &reason, failure_details);
						}
					},
		...
		if let Some(height) = height_opt {
				...
				payment.htlcs.retain(|htlc| {
					...
					if height >= htlc.cltv_expiry - HTLC_FAIL_BACK_BUFFER {
						...
						timed_out_htlcs.push((HTLCSource::PreviousHopData(htlc.prev_hop.clone()), payment_hash.clone(),
							...
							HTLCHandlingFailureDetails {
								htlc_type: HTLCHandlingType::Receive{ payment_hash: payment_hash.clone() },
								reason: reason.into(),
							}));
						false
					} else { true }
			...
			intercepted_htlcs.retain(|_, htlc| {
				if height >= htlc.forward_info.outgoing_cltv_value - HTLC_FAIL_BACK_BUFFER {
					...
					let reason = LocalHTLCFailureReason::ForwardExpiryBuffer;
					timed_out_htlcs.push((prev_hop_data, htlc.forward_info.payment_hash,
							HTLCFailReason::from_failure_code(reason),
							HTLCHandlingFailureDetails {
								htlc_type: HTLCHandlingType::Intercept{ intercept_id: requested_forward_scid },
								reason: reason.into(),
							}
					));
					let logger = WithContext::from(
	...
	L::Target: Logger,
{
	fn read<Reader: io::Read>(reader: &mut Reader, mut args: ChannelManagerReadArgs<'a, M, T, ES, NS, SP, F, R, MR, L>) -> Result<Self, DecodeError> {
		...
		for htlc_source in failed_htlcs.drain(..) {
			...
			let reason = HTLCFailReason::from_failure_code(failure_reason);
			channel_manager.fail_htlc_backwards_internal(&source, &payment_hash, &reason, failure_details);
		}
