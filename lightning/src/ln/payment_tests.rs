...
fn failed_probe_yields_event() {
	...
	let probe_event = SendEvent::from_commitment_update(nodes[1].node.get_our_node_id(), channel_id, updates);
...
fn onchain_failed_probe_yields_event() {
	...
	check_added_monitors!(nodes[0], 1);
	let updates = get_htlc_update_msgs!(nodes[0], nodes[1].node.get_our_node_id());
	let probe_event = SendEvent::from_commitment_update(nodes[1].node.get_our_node_id(), chan_id, updates);
	nodes[1].node.handle_update_add_htlc(nodes[0].node.get_our_node_id(), &probe_event.msgs[0]);
	check_added_monitors!(nodes[1], 0);
	commitment_signed_dance!(nodes[1], nodes[0], probe_event.commitment_msg, false);
	expect_pending_htlcs_forwardable!(nodes[1]);
...
fn abandoned_send_payment_idempotent() {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1], [HTLCHandlingType::ReceiveFailed { payment_hash: first_payment_hash }]);
...
fn do_test_intercepted_payment(test: InterceptTest) {
	...
	if test == InterceptTest::Fail {
		...
		expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1], vec![HTLCHandlingType::InvalidForward { requested_forward_scid: intercept_scid }]);
		check_added_monitors!(nodes[1], 1);
...
fn auto_retry_partial_failure() {
	...
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &as_second_htlc_updates.commitment_msg);
	check_added_monitors!(nodes[1], 1);
	...
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &as_third_cs);
	check_added_monitors!(nodes[1], 1);
	...
	nodes[0].node.handle_commitment_signed_batch_test(nodes[1].node.get_our_node_id(), &bs_second_claim_update.commitment_signed);
	check_added_monitors!(nodes[0], 1);
	...
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &as_fourth_cs);
	check_added_monitors!(nodes[1], 1);
...
fn fails_paying_after_rejected_by_payee() {
	...
	nodes[1].node.fail_htlc_backwards(&payment_hash);
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(nodes[1], [HTLCHandlingType::ReceiveFailed { payment_hash }]);
...
fn no_extra_retries_on_back_to_back_fail() {
	...
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &first_htlc_updates.commitment_msg);
	check_added_monitors!(nodes[1], 1);
	...
	nodes[1].node.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &second_htlc_updates.commitment_msg);
	check_added_monitors!(nodes[1], 1);
...
fn do_claim_from_closed_chan(fail_payment: bool) {
	...
	if fail_payment {
		...
		let reason = HTLCHandlingType::ReceiveFailed { payment_hash };
...
fn do_test_custom_tlvs(spontaneous: bool, even_tlvs: bool, known_tlvs: bool) {
	...
	match (known_tlvs, even_tlvs) {
		...
		(false, true) => {
			nodes[1].node.claim_funds(our_payment_preimage);
			let expected_destinations = vec![HTLCHandlingType::ReceiveFailed { payment_hash: our_payment_hash }];
...
fn do_test_payment_metadata_consistency(do_reload: bool, do_modify: bool) {
	...
	if do_modify {
		...
		expect_pending_htlcs_forwardable_conditions(nodes[2].node.get_and_clear_pending_events(),
			&[HTLCHandlingType::ForwardFailed { node_id: Some(nodes[3].node.get_our_node_id()), channel_id: cd_channel_used }]);
	} else {
		expect_pending_htlcs_forwardable!(nodes[3]);
...
fn  test_htlc_forward_considers_anchor_outputs_value() {
	...
	expect_pending_htlcs_forwardable!(nodes[1]);
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(&nodes[1], vec![HTLCHandlingType::ForwardFailed {
		node_id: Some(nodes[2].node.get_our_node_id()),
...
fn test_non_strict_forwarding() {
	...
	expect_pending_htlcs_forwardable_conditions(nodes[1].node.get_and_clear_pending_events(),
		&[HTLCHandlingType::ForwardFailed { node_id: Some(nodes[2].node.get_our_node_id()), channel_id: routed_channel_id }]);
